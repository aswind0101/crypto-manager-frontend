...
function buildOrderflowSummary(orderbook, recentTrades) {
  ...
  for (const t of recentTrades || []) {
    ...
    // Bybit recent-trade thường có price ở key 'price' hoặc 'p'
    const priceRaw = t.price ?? t.p ?? t.P ?? null;
  ...
  if (tsWithPrice.length >= 10) {
    ...
    if (win3m.length >= 10) {
      ...
      // Khi có aggression 1 phía nhưng giá không follow-through tương ứng.
      // price_change nhỏ nhưng delta lớn => absorption
      const priceChange = last.price - first.price;
      const absDelta = Math.abs(delta3);
      const absPriceChange = Math.abs(priceChange);

      // scale: absDelta và absPriceChange tương quan thô (không có tick value), dùng ratio heuristic
      // Nếu delta lớn nhưng giá không đi => absorption.
      // Điều kiện: absDelta > 0 và absPriceChange/rangeP < 0.15
      if (absDelta > 0 && (absPriceChange / rangeP) < 0.15) {
        // Nếu delta3 > 0 mà giá không tăng đáng kể => buy absorption (bị hấp thụ ở offer)
        if (delta3 > 0 && priceChange <= 0) absorption_flag = "buy_absorption";
        // Nếu delta3 < 0 mà giá không giảm đáng kể => sell absorption (bị hấp thụ ở bid)
        if (delta3 < 0 && priceChange >= 0) absorption_flag = "sell_absorption";

        // strength: delta lớn + price move nhỏ => mạnh
        const strength = Math.min(1, (absDelta / (absDelta + 1)) * (1 - Math.min(1, absPriceChange / rangeP)));
...
function markSweepThenAbsorb(fullSnap) {
  ...
  for (const [sym, ltf] of Object.entries(ltfSymbols)) {
    ...
    const lastSweep = Array.isArray(sweeps) && sweeps.length ? sweeps[sweeps.length - 1] : null;

    const er = ltf?.orderflow_summary?.early_reversal;
    if (!er) continue;

    const hasAbsorb = !!er.absorption_flag;
    const hasDiv = !!er.delta_divergence_flag;

    // Điều kiện "gần đây": sweep trong vòng 2 giờ (tùy style, bạn đổi được)
    const now = fullSnap?.generated_at || Date.now();
    const recentSweep =
      lastSweep?.ts_closed && Number.isFinite(lastSweep.ts_closed)
        ? (now - lastSweep.ts_closed) <= 2 * 60 * 60 * 1000
        : false;

    er.sweep_then_absorb_flag = Boolean(recentSweep && (hasAbsorb || hasDiv));
  }
