...
// v3.1: detect BOS đơn giản dựa trên swings
function detectRecentBOS(swings, lastClose, maxLookback = 8) {
  if (!Array.isArray(swings) || swings.length < 3) return [];

  const recent = swings.slice(-maxLookback);
  const result = [];

  // BOS bullish: close phá vỡ swing high quan trọng gần nhất
...
function computePriceStructureTF(parsed, tfLabel) {
  ...
  const recent_bos = detectRecentBOS(swings, lastClose, 8);

  const rangeWidthPct = ((high - low) / low) * 100;
