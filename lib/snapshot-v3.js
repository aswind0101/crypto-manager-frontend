...
function buildOrderflowSummary(orderbook, recentTrades) {
  ...
  if (tsValid.length) {
    ...
    for (const tr of tsValid) {
      ...
      else if (tr.side === "Sell") bucket.sell += tr.qty;
    }
  ...
  if (tsWithPrice.length >= 6) {
    ...
    // fallback 5 phút nếu 3 phút quá ít trade
    if (win3m.length < 6) {
      const fiveMinAgo = lastTs - 300_000;
      win3m = tsWithPrice.filter((t) => t.ts >= fiveMinAgo);
    }

    // DEBUG: kiểm tra window 3m có đủ trades và side có vào được buy/sell không
    if (win3m.length && win3m.length < 6) {
      console.log("[DEBUG][ER] win3m small:", win3m.length, "lastTs:", lastTs, "threeMinAgo:", threeMinAgo);
    }
    ...
    if (win3m.length >= 6) {
      ...
      console.log("[DEBUG][ER] win3m:", win3m.length, "buy3:", buy3, "sell3:", sell3, "delta3:", delta3);

      // Ngưỡng "near extreme": dùng range của window
      const rangeP = Math.max(1e-12, maxP - minP);
      const nearTop = last.price >= maxP - 0.2 * rangeP;
      const nearBot = last.price <= minP + 0.2 * rangeP;

      // -------- Delta flip (micro early reversal):
      // Ý tưởng: 3m vẫn cùng hướng nhưng 1m đã flip ngược -> dấu hiệu hụt hơi sớm
      ...
      // Nếu giá đang nearTop mà delta1 âm trong khi delta3 dương -> bearish early
      // Nếu giá đang nearBot mà delta1 dương trong khi delta3 âm -> bullish early
      const deltaFlipBear = nearTop && delta3 > 0 && delta1 < 0;
      const deltaFlipBull = nearBot && delta3 < 0 && delta1 > 0;

      // Ưu tiên delta flip vì nó “sớm” hơn divergence kiểu delta3 đổi dấu
      if (deltaFlipBear) delta_divergence_flag = "bearish";
      else if (deltaFlipBull) delta_divergence_flag = "bullish";

      // -------- Delta divergence (heuristic cũ):
      // - Bearish divergence: giá gần đỉnh nhưng delta3 <= 0
      // - Bullish divergence: giá gần đáy nhưng delta3 >= 0
      ...
      // -------- Exhaustion (early top/bottom catching):
      // Điều kiện: near extreme + delta cực lệch + giá không follow mạnh "đủ tương xứng"
      // 1) delta dominance ratio
      const dominance = totalVol3 > 0 ? Math.abs(delta3) / totalVol3 : 0;

      // 2) price follow-through ratio (độ "đi" của giá so với range window)
      const follow = rangeP > 0 ? (absPriceChange / rangeP) : 0;

      // 3) delta burst: delta tập trung vào 15s bucket lớn (dùng buckets nhanh ngay trong win3m)
      const bucketMs = 15_000;
      const buckets = new Map();
      for (const tr of win3m) {
      ...
      // burst ratio: burst lớn chiếm bao nhiêu % total cùng phía
      const buyTotal = buy3;
      const sellTotal = sell3;
      const buyBurstRatio = buyTotal > 0 ? maxBuyBurst / buyTotal : 0;
      const sellBurstRatio = sellTotal > 0 ? maxSellBurst / sellTotal : 0;

      // Heuristic thresholds (thực dụng):
      // - dominance >= 0.85: 1 phía gần như áp đảo
      // - follow <= 0.55: giá không "đi" tương xứng (hoặc bắt đầu hụt)
      // - burstRatio >= 0.55: dòng tiền dồn vào burst (hay gặp ở blow-off)
      ...
      // ===== Debug snapshot để biết vì sao ER không trigger =====
      early_reversal_debug = {
        ...
        sell3,
        delta3,
        // delta1 chỉ có sau khi tính delta flip
        delta1,
        priceChange,
        absPriceChange,
        priceStalled,
  ...
  return {
    ...
    early_reversal: {
      ...
      exhaustion_flag,      // bool (sẽ set ở Step 4)
...
export async function buildLtfSnapshotV3(symbols) {
  ...
  for (const sym of symbols) {
    ...
    const orderflow_summary = buildOrderflowSummary(orderbook, recentTradesBuffered);
