...
function markSweepThenAbsorb(fullSnap) {
  ...
  for (const [sym, ltf] of Object.entries(ltfSymbols)) {
    ...
    // ưu tiên sweep phù hợp hướng divergence nếu có
    if (Array.isArray(sweeps) && sweeps.length) {
      const wantSide =
        er?.delta_divergence_flag === "bullish" ? "sell_side" :
          er?.delta_divergence_flag === "bearish" ? "buy_side" : null;

      if (wantSide) {
        const candidates = sweeps.filter(s => s?.side === wantSide && Number.isFinite(s?.ts_closed));
        if (candidates.length) {
          candidates.sort((a, b) => (a.ts_closed || 0) - (b.ts_closed || 0));
          lastSweep = candidates[candidates.length - 1];
        }
      }
    }
    ...
    // Intraday thực dụng: sweep trong 90 phút
    const now = fullSnap?.generated_at || Date.now();
    const recentSweep =
      lastSweep?.ts_closed && Number.isFinite(lastSweep.ts_closed)
        ? (now - lastSweep.ts_closed) <= 90 * 60 * 1000
        : false;

    // Match hướng:
    // - Bullish divergence/absorption hữu ích nhất khi vừa sweep "sell_side" (quét đáy)
    // - Bearish divergence/absorption hữu ích nhất khi vừa sweep "buy_side" (quét đỉnh)
    ...
    const absorbOK =
      (er.absorption_flag === "sell_absorption" && sweepSide === "sell_side") || // bán mạnh nhưng không xuống → đáy
      (er.absorption_flag === "buy_absorption" && sweepSide === "buy_side");     // mua mạnh nhưng không lên → đỉnh
