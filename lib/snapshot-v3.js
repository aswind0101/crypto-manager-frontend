...
function buildOrderflowSummary(orderbook, recentTrades) {
  ...
  if (tsWithPrice.length >= 6) {
    ...
    if (win3m.length >= 6) {
      ...
      // Tính delta1m ngay trong window win3m (để đồng bộ nguồn dữ liệu)
      const oneMinAgo = lastTs - 60_000;
      let buy1 = 0;
      let sell1 = 0;
      for (const tr of win3m) {
      ...
      // Nếu giá đang nearTop mà delta1 đã âm trong khi delta3 dương -> bearish early
      // Nếu giá đang nearBot mà delta1 đã dương trong khi delta3 âm -> bullish early
