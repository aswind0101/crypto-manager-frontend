...
// ====================== LTF TRADE ROLLING BUFFER (5m) ======================
// Mục tiêu: recentTrades API có thể chỉ cover vài chục giây nếu market dày.
// Buffer giúp tích lũy trades qua nhiều lần snapshot để đủ span thời gian cho ER.
const __tradeBufferBySymbol = new Map();
// ====================== SWEEP HYSTERESIS MEMORY (per symbol) ======================
// Mục tiêu: distPct quanh ngưỡng sẽ làm distOK flip-flop theo từng snapshot.
// Ta lưu distOK trước đó trong RAM để tạo hysteresis (giữ trạng thái ổn định).
const __sweepHysteresisBySymbol = new Map();
// value: { distOK: boolean, updatedAt: number }


function __toMsTsFromTrade(t) {
  const tsCandidate =
    t?.time ?? t?.T ?? t?.ts ?? t?.tradeTimeMs ?? t?.execTime ?? t?.execTimeMs ?? t?.timestamp ?? null;
  let ts = tsCandidate != null ? Number(tsCandidate) : null;
  if (Number.isFinite(ts) && ts > 0 && ts < 1e12) ts = ts * 1000; // normalize seconds->ms
  return Number.isFinite(ts) && ts > 0 ? ts : null;
}

function __tradeId(t) {
  // Ưu tiên execId, fallback seq, fallback time+price+size
  return (
    t?.execId ??
    t?.exec_id ??
    t?.id ??
    (t?.seq != null ? `seq:${t.seq}` : null) ??
    (() => {
      const ts = __toMsTsFromTrade(t);
      const p = t?.price ?? t?.p ?? t?.P ?? "";
      const sz = t?.size ?? t?.qty ?? t?.v ?? "";
      return ts ? `ts:${ts}|p:${p}|s:${sz}` : null;
    })()
  );
}

function updateTradeBuffer(symbol, newTrades, keepMs = 5 * 60 * 1000) {
  if (!symbol) return Array.isArray(newTrades) ? newTrades : [];
  const now = Date.now();

  const prev = __tradeBufferBySymbol.get(symbol) || [];
  const seen = new Set();

  // seed with prev (dedupe)
  const merged = [];
  for (const t of prev) {
    const id = __tradeId(t);
  ...
  // sort by time asc
  merged.sort((a, b) => (__toMsTsFromTrade(a) || 0) - (__toMsTsFromTrade(b) || 0));

  // keep only last keepMs window (by timestamp if possible, else by count)
  ...
  // cap size to avoid memory blowup
  const capped = filtered.slice(-5000);

  __tradeBufferBySymbol.set(symbol, capped);
  return capped;
}
...
function buildOrderflowSummary(orderbook, recentTrades, ctx = null) {
  ...
  for (const t of recentTrades || []) {
    ...
    const sideRaw = (t.side || t.S || "").toString().trim();
    let side = null;
    const sLower = sideRaw.toLowerCase();
    if (sLower.includes("buy")) side = "Buy";
    else if (sLower.includes("sell")) side = "Sell";
    if (!side) continue;
  ...
  if (tsValid.length >= 2) {
    ...
    if (tsMax > tsMin) windowSec = (tsMax - tsMin) / 1000;
  }
  ...
  if (tsValid.length) {
    ...
    let buy1 = 0, sell1 = 0, buy3 = 0, sell3 = 0;
  ...
  if (tsValid.length) {
    ...
    for (const tr of tsValid) {
      ...
      if (tr.side === "Buy") bucket.buy += tr.qty;
      else bucket.sell += tr.qty;
    }

    let maxB = 0, maxS = 0;
    for (const bucket of buckets.values()) {
      if (bucket.buy > maxB) maxB = bucket.buy;
      if (bucket.sell > maxS) maxS = bucket.sell;
    }
  ...
  if (delta3m != null) {
    if (delta3m > 0 && (delta1m ?? 0) > 0) shortTermPressure = "buy_pressure";
    else if (delta3m < 0 && (delta1m ?? 0) < 0) shortTermPressure = "sell_pressure";
  }

  // mm_mode
  let mm_mode = "neutral";
  if (Math.abs(imbalance) < 0.05 && aggression === "neutral") mm_mode = "range_holding";
  else if (imbalance > 0.1 && aggression === "aggressive_buy") mm_mode = "pushing_up";
  else if (imbalance < -0.1 && aggression === "aggressive_sell") mm_mode = "pushing_down";

  // ================== EARLY REVERSAL SIGNALS (v3.4+) ==================
  ...
  let absorption_strength = null;     // 0..1
  let sweep_then_absorb_flag = false; // context-aware set below
  let exhaustion_flag = null;         // "buy_exhaustion" | "sell_exhaustion" | null

  // ✅ ALWAYS create debug object (không còn null)
  let early_reversal_debug = {
    reason: null,
    relevant_sweep: null,
    sweep_context: null,
    last_price: tsWithPrice.length ? tsWithPrice[tsWithPrice.length - 1].price : null,
    nearTop: null,
    nearBot: null,
  };

  // ---------- Sweep picking helpers (same idea as markSweepThenAbsorb) ----------
  const normalizeSweepSide = (side) => {
    if (!side) return null;
    const s = String(side).toLowerCase();
    if (s.includes("sell")) return "sell_side";
    if (s.includes("buy")) return "buy_side";
    if (s.includes("down")) return "sell_side";
    if (s.includes("up")) return "buy_side";
    return null;
  };
  ...
  function pickRelevantSweep(sweeps, nowMs, currentPrice) {
    if (!Array.isArray(sweeps) || !sweeps.length || !Number.isFinite(currentPrice)) return null;

    const norm = sweeps
      .map((x) => ({
        ...x,
        ts_closed: normalizeTsMs(x?.ts_closed),
        side: normalizeSweepSide(x?.side),
        swept_level: Number(x?.swept_level),
        tf: x?.tf,
      }))
      .filter((x) => x.ts_closed && x.side && Number.isFinite(x.swept_level));
    ...
    for (const s of norm) {
      const tf = ["H1", "H4", "D1"].includes(s.tf) ? s.tf : "H1";
      const w = tfWindowMs(tf);

      const age = nowMs - s.ts_closed;
      if (age < 0 || age > w) continue;

      const distPct = Math.abs(s.swept_level - currentPrice) / currentPrice;
      if (distPct > 0.03) continue;

      const timeScore = 1 - (age / w);
      const distScore = 1 - Math.min(1, distPct / 0.03);
      const score = 0.65 * timeScore + 0.35 * distScore;
  ...
  const currentPrice = Number(ctx?.currentPrice ?? early_reversal_debug.last_price);
  const relevantSweep = pickRelevantSweep(ctx?.sweep_events || [], nowMs, currentPrice);

  if (relevantSweep) {
    early_reversal_debug.relevant_sweep = {
      tf: relevantSweep.tf,
      side: relevantSweep.side,
      swept_level: relevantSweep.swept_level,
      ts_closed: relevantSweep.ts_closed,
    };
  }

  if (tsWithPrice.length < 6) {
    early_reversal_debug.reason = "insufficient_trades_ts_price";
  } else {
    const lastTs = tsWithPrice[tsWithPrice.length - 1].ts;

    const threeMinAgo = lastTs - 180_000;
    let win3m = tsWithPrice.filter((t) => t.ts >= threeMinAgo);

    if (win3m.length < 6) {
      const fiveMinAgo = lastTs - 300_000;
      win3m = tsWithPrice.filter((t) => t.ts >= fiveMinAgo);
    }

    if (win3m.length < 6) {
      early_reversal_debug.reason = "insufficient_win3m";
    } else {
      const first = win3m[0];
      const last = win3m[win3m.length - 1];
      const spanMs = (last.ts || 0) - (first.ts || 0);
      ...
      // ✅ NEO nearTop/nearBot theo swept_level nếu có relevantSweep
      let nearTop = false;
      let nearBot = false;

      const tol = Math.max(0.0012 * last.price, 0.18 * rangeP); // tune nếu muốn

      if (relevantSweep?.swept_level && relevantSweep?.side) {
        if (relevantSweep.side === "buy_side") {
          // quét đỉnh -> nearTop quanh swept_level
          nearTop = last.price >= (Number(relevantSweep.swept_level) - tol);
        } else if (relevantSweep.side === "sell_side") {
          // quét đáy -> nearBot quanh swept_level
          nearBot = last.price <= (Number(relevantSweep.swept_level) + tol);
        }
      } else {
        // fallback logic cũ
        nearTop = last.price >= maxP - 0.2 * rangeP;
        nearBot = last.price <= minP + 0.2 * rangeP;
      }

      // Delta flip
      const recentMs = Math.max(15_000, Math.min(60_000, Math.floor(spanMs * 0.2)));
      const oneMinAgo = lastTs - recentMs;
      ...
      const delta1 = buy1 - sell1;

      const deltaFlipBear = nearTop && delta3 > 0 && delta1 < 0;
      const deltaFlipBull = nearBot && delta3 < 0 && delta1 > 0;

      if (deltaFlipBear) delta_divergence_flag = "bearish";
      else if (deltaFlipBull) delta_divergence_flag = "bullish";
      ...
      const priceStalled = (absPriceChange / rangeP) < 0.30;
      const movedAgainstDelta =
        (delta3 > 0 && priceChange <= 0) || (delta3 < 0 && priceChange >= 0);

      if (deltaIsMeaningful && (priceStalled || movedAgainstDelta)) {
        if (delta3 > 0) absorption_flag = "buy_absorption";
        if (delta3 < 0) absorption_flag = "sell_absorption";
      ...
      // Exhaustion
      const dominance = totalVol3 > 0 ? Math.abs(delta3) / totalVol3 : 0;
      const follow = rangeP > 0 ? (Math.abs(last.price - first.price) / rangeP) : 0;

      const bucketMs = 15_000;
      const buckets = new Map();
      for (const tr of win3m) {
        const k = Math.floor(tr.ts / bucketMs);
      ...
      let maxBuyBurst = 0, maxSellBurst = 0;
      for (const bucket of buckets.values()) {
        if (bucket.buy > maxBuyBurst) maxBuyBurst = bucket.buy;
        if (bucket.sell > maxSellBurst) maxSellBurst = bucket.sell;
      }

      const buyBurstRatio = buy3 > 0 ? maxBuyBurst / buy3 : 0;
      const sellBurstRatio = sell3 > 0 ? maxSellBurst / sell3 : 0;

      const domStrong = dominance >= 0.85;
      const followWeak = follow <= 0.55;
      const buyBurst = buyBurstRatio >= 0.55;
      const sellBurst = sellBurstRatio >= 0.55;

      if (nearTop && delta3 > 0 && domStrong && (followWeak || buyBurst)) {
        exhaustion_flag = "buy_exhaustion";
      } else if (nearBot && delta3 < 0 && domStrong && (followWeak || sellBurst)) {
        exhaustion_flag = "sell_exhaustion";
      }

      // ✅ sweep_then_absorb_flag (context-aware)
      // - Có reclaim qua swept_level
      // - Gần swept_level
      // - Evidence aligned (ưu tiên đúng hướng đảo chiều)
      if (relevantSweep?.swept_level && relevantSweep?.side) {
        const lvl = Number(relevantSweep.swept_level);
        const px = Number(last.price);
        const distPct = Math.abs(px - lvl) / px;

        const reclaimOK =
          (relevantSweep.side === "buy_side" && px < lvl) ||   // quét đỉnh xong -> đóng/reclaim dưới level
          (relevantSweep.side === "sell_side" && px > lvl);    // quét đáy xong -> đóng/reclaim trên level

        // ==== OPTION A: distOK theo TF + age gate theo TF ====
        const distLimit =
          (relevantSweep.tf === "H1") ? 0.006 :
            (relevantSweep.tf === "H4") ? 0.015 :
              (relevantSweep.tf === "D1") ? 0.025 :
                0.010;

        const distOK = distPct <= distLimit;

        // Age gate: chặn sweep quá cũ theo TF
        const ageMs = nowMs - Number(relevantSweep.ts_closed);
        const ageLimitMs =
          (relevantSweep.tf === "H1") ? 2 * 60 * 60 * 1000 :        // 2 giờ
            (relevantSweep.tf === "H4") ? 12 * 60 * 60 * 1000 :       // 12 giờ
              (relevantSweep.tf === "D1") ? 3 * 24 * 60 * 60 * 1000 :   // 3 ngày
                6 * 60 * 60 * 1000;                                       // default 6 giờ

        const ageOK = Number.isFinite(ageMs) ? (ageMs >= 0 && ageMs <= ageLimitMs) : true;

        const divAligned =
          (delta_divergence_flag === "bearish" && relevantSweep.side === "buy_side") ||
          (delta_divergence_flag === "bullish" && relevantSweep.side === "sell_side");

        // Absorption aligned theo hướng đảo chiều:
        // - quét đỉnh => kỳ vọng sell_absorption
        // - quét đáy => kỳ vọng buy_absorption
        const absorbAligned =
          (absorption_flag === "sell_absorption" && relevantSweep.side === "buy_side") ||
          (absorption_flag === "buy_absorption" && relevantSweep.side === "sell_side");

        const absorbStrengthOK =
          absorption_strength == null ? true : absorption_strength >= 0.45;
        // Evidence #3: retest/reclaim follow-through (dùng khi divergence/absorption chưa bật)
        // Ý tưởng:
        // - sell_side sweep (quét đáy): sau reclaim, nếu micro-window có delta3 > 0
        //   và giá không bị kéo xuống mạnh (follow không quá yếu) => bullish follow-through
        // - buy_side sweep (quét đỉnh): sau reclaim, nếu micro-window có delta3 < 0
        //   và giá không bị kéo lên mạnh => bearish follow-through
        //
        // Dùng dominance để tránh tín hiệu quá "mỏng".
        const domOK = Number.isFinite(dominance) ? (dominance >= 0.10) : true; // bạn đang thấy dominance ~0.10-0.30 là bình thường
        const followOK = Number.isFinite(follow) ? (follow >= 0.20) : true;   // tránh trường hợp giá đứng im hoàn toàn
        const flowAligned =
          (relevantSweep.side === "sell_side" && delta3 > 0) ||
          (relevantSweep.side === "buy_side" && delta3 < 0);

        const retestAligned = Boolean(domOK && followOK && flowAligned);

        sweep_then_absorb_flag = Boolean(
          reclaimOK && distOK && ageOK &&
          (divAligned || (absorbAligned && absorbStrengthOK) || retestAligned)
        );

        early_reversal_debug.sweep_context = {
          reclaimOK,
          distPct,
          distLimit,
          distOK,
        ...
        sweep_then_absorb_flag = false;
      }

      early_reversal_debug = {
        ...early_reversal_debug,
        win_len: win3m.length,
        maxP,
        minP,
        rangeP,
        ...
        absPriceChange: Math.abs(last.price - first.price),
        totalVol3: buy3 + sell3,
        dominance,
        follow,
        buyBurstRatio,
        sellBurstRatio,
  ...
  return {
    ...
    early_reversal: {
      ...
      absorption_flag,
      absorption_strength,
      sweep_then_absorb_flag,
      exhaustion_flag,
      debug: early_reversal_debug,
    },
  };
}
...
export async function buildSnapshotV3(symbols) {
  ...
  for (const sym of symbols) {
    ...
    // Rolling buffer trades (5m) để ER có span thời gian đủ dài
    const recentTradesBuffered = updateTradeBuffer(sym, bybitData.recent_trades, 5 * 60 * 1000);

    const currentPriceHTF =
      Number(bybitData?.ticker?.lastPrice ?? bybitData?.ticker?.last ?? bybitData?.ticker?.markPrice ?? NaN);

    const orderflow_summary = buildOrderflowSummary(
      bybitData.orderbook,
      recentTradesBuffered,
      {
        now: generatedAt,
        currentPrice: Number.isFinite(currentPriceHTF) ? currentPriceHTF : null,
        sweep_events: Array.isArray(sweep_events) ? sweep_events : [],
      }
    );
...
export async function buildLtfSnapshotV3(symbols, opts = {}) {
  const generatedAt = Date.now();
  const htfSweepsBySymbol = opts?.htfSweepsBySymbol || {};
  ...
  for (const sym of symbols) {
    ...
    // Rolling buffer trades (5m) để ER có span thời gian đủ dài
    const recentTradesBuffered = updateTradeBuffer(sym, recent_trades, 5 * 60 * 1000);
    console.log(
      `[snapshot-v3.1][BUFFER] ${sym} recent_trades=${(recent_trades || []).length} buffered=${(recentTradesBuffered || []).length}`
    );
    const currentPriceLTF =
      Number(ticker?.lastPrice ?? ticker?.last ?? ticker?.markPrice ?? NaN);
    ...
    const orderflow_summary = buildOrderflowSummary(
      ...
      {
        now: generatedAt,
        currentPrice: Number.isFinite(currentPriceLTF) ? currentPriceLTF : null,
        sweep_events: Array.isArray(htfSweepsBySymbol[sym]) ? htfSweepsBySymbol[sym] : [],
      }
    );
...
function markSweepThenAbsorb(fullSnap) {
  ...
  for (const [sym, ltf] of Object.entries(ltfSymbols)) {
    ...
    const sweeps = Array.isArray(htf?.sweep_events) ? htf.sweep_events : [];
    const now = fullSnap?.generated_at || Date.now();

    // currentPrice: ưu tiên last_price trong ER debug, fallback ticker HTF
    const currentPrice =
      ltf?.orderflow_summary?.early_reversal?.debug?.last_price ??
      htf?.ticker?.last ??
      htf?.ticker?.lastPrice ??
      null;

    const lastSweep = pickRelevantSweep(sweeps, now, Number(currentPrice));

    // Ensure debug object luôn tồn tại để ghi relevant_sweep/sweep_context
    if (!er.debug) er.debug = { reason: "no_debug_from_orderflow" };

    if (!lastSweep) {
      er.sweep_then_absorb_flag = false;
      er.debug.relevant_sweep = null;
      er.debug.sweep_context = {
        reason: "no_relevant_sweep",
      };
      // cập nhật hysteresis memory
      __sweepHysteresisBySymbol.set(sym, { distOK: false, updatedAt: now });
      continue;
    }

    // attach relevant_sweep
    er.debug.relevant_sweep = {
      tf: lastSweep.tf,
      side: lastSweep.side,
      swept_level: lastSweep.swept_level,
      ts_closed: lastSweep.ts_closed,
    };

    const sweepSide = lastSweep.side; // "sell_side" | "buy_side"
    const lvl = Number(lastSweep.swept_level);
    const px = Number(currentPrice);

    // ===== Context gates: reclaim + dist (Option A) + age =====
    const reclaimOK =
      (sweepSide === "buy_side" && px < lvl) ||   // quét đỉnh -> reclaim dưới level
      (sweepSide === "sell_side" && px > lvl);    // quét đáy -> reclaim trên level

    const distPct = Math.abs(px - lvl) / px;

    // Option A: dist limit theo TF
    const distLimitBase =
      (lastSweep.tf === "H1") ? 0.006 :
      (lastSweep.tf === "H4") ? 0.015 :
      (lastSweep.tf === "D1") ? 0.025 :
      0.010;

    // Hysteresis: nếu snapshot gần đây distOK=true thì nới nhẹ để tránh flip-flop
    const prev = __sweepHysteresisBySymbol.get(sym);
    const prevFresh = prev && Number.isFinite(prev.updatedAt) && (now - prev.updatedAt <= 10 * 60 * 1000);
    const prevDistOK = prevFresh ? Boolean(prev.distOK) : false;

    const distLimit = prevDistOK ? distLimitBase * 1.08 : distLimitBase;
    const distOK = distPct <= distLimit;

    // Age gate theo TF
    const tsClosed = Number(lastSweep.ts_closed);
    const ageMs = Number.isFinite(tsClosed) ? (now - tsClosed) : null;

    const ageLimitMs =
      (lastSweep.tf === "H1") ? 2 * 60 * 60 * 1000 :        // 2h
      (lastSweep.tf === "H4") ? 12 * 60 * 60 * 1000 :       // 12h
      (lastSweep.tf === "D1") ? 3 * 24 * 60 * 60 * 1000 :   // 3d
      6 * 60 * 60 * 1000;

    const ageOK = (ageMs == null) ? true : (ageMs >= 0 && ageMs <= ageLimitMs);

    // ===== Evidence: divergence/absorption alignment =====
    const divOK =
      (er.delta_divergence_flag === "bullish" && sweepSide === "sell_side") ||
      (er.delta_divergence_flag === "bearish" && sweepSide === "buy_side");

    // FIX quan trọng: absorption alignment phải theo hướng đảo chiều sau sweep
    // sell_side sweep (quét đáy) -> kỳ vọng buy_absorption
    // buy_side sweep (quét đỉnh) -> kỳ vọng sell_absorption
    const absorbOK =
      (er.absorption_flag === "buy_absorption" && sweepSide === "sell_side") ||
      (er.absorption_flag === "sell_absorption" && sweepSide === "buy_side");

    // ===== Evidence #3: retestAligned (micro flip) =====
    const d = er.debug || {};
    const delta3 = Number(d.delta3);
    const delta1 = Number(d.delta1);
    const dominance = Number(d.dominance);
    const follow = Number(d.follow);

    const domOK = Number.isFinite(dominance) ? (dominance >= 0.10) : true;
    const followOK = Number.isFinite(follow) ? (follow >= 0.20) : true;

    // micro flip:
    // - sell_side sweep (quét đáy): muốn thấy delta1 bớt âm hơn delta3 (giảm lực sell) hoặc delta1 > 0
    // - buy_side sweep (quét đỉnh): muốn thấy delta1 bớt dương hơn delta3 hoặc delta1 < 0
    const deltaFlipUp =
      (sweepSide === "sell_side") &&
      (Number.isFinite(delta1) && Number.isFinite(delta3)) &&
      (delta1 > 0 || (delta3 < 0 && delta1 > delta3 * 0.35));
    ...
    const retestAligned = Boolean(domOK && followOK && (deltaFlipUp || deltaFlipDown));

    // ===== Final decision =====
    // Cấu trúc: context gates phải pass, sau đó cần ít nhất 1 evidence trong 3 cái (div/absorb/retest)
    er.sweep_then_absorb_flag = Boolean(
      reclaimOK && distOK && ageOK && (divOK || absorbOK || retestAligned)
    );

    // attach sweep_context for tuning
    er.debug.sweep_context = {
      reclaimOK,
      distPct,
      distLimitBase,
    ...
    __sweepHysteresisBySymbol.set(sym, { distOK: Boolean(distOK), updatedAt: now });
  }
...
export async function buildFullSnapshotV3(symbols) {
  ...
  // 1) HTF trước (để có sweep_events)
  const htfSnap = await buildSnapshotV3(symbols);

  // 2) Build map sweeps theo symbol
  const htfSymbols = htfSnap?.per_exchange?.bybit?.symbols || [];
  const htfSweepsBySymbol = {};
  for (const s of htfSymbols) {
    if (!s?.symbol) continue;
    htfSweepsBySymbol[s.symbol] = Array.isArray(s.sweep_events) ? s.sweep_events : [];
  }

  // 3) LTF sau, truyền sweeps vào
  const ltfSnap = await buildLtfSnapshotV3(symbols, { htfSweepsBySymbol });

  // 4) Merge giữ nguyên format cũ
  const merged = {
    ...(htfSnap || {}),
    schema: {
  ...
  // markSweepThenAbsorb vẫn có thể giữ để “soi” consistency,
  // nhưng sweep_then_absorb_flag giờ đã tính ngay trong buildOrderflowSummary()
