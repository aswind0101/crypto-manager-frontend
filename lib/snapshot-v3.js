...
export async function buildLtfSnapshotV3(symbols) {
  ...
  for (const sym of symbols) {
    ...
    // --- NEW: LTF trigger readiness (default dùng M5 cho timing) ---
    // --- NEW: LTF trigger readiness (default dùng M5 cho timing) ---
    // Bybit thường trả về candle đang chạy ở cuối list => is_last_closed gần như luôn false.
    // Vì vậy readiness gate nên dựa trên last_closed_ts + tf_ms (+ safety) thay vì is_last_closed.

    const m5 = candleStatusLtf?.["5"] || {};
    const tfMs = Number(m5.tf_ms) || 5 * 60 * 1000;

    // safetyMs: buffer chống lag cập nhật kline/indicator (2–5s là hợp lý)
    const safetyMs = 3000;

    // last_closed_ts trong candle_status là "open ts" của cây đã đóng gần nhất
    const lastClosedOpen = Number(m5.last_closed_ts);

    // Điều kiện 1: phải có last_closed_ts hợp lệ
    const hasProof = Number.isFinite(lastClosedOpen) && lastClosedOpen > 0;

    // Điều kiện 2: đã qua thời điểm close của cây đó (open + tfMs) + safety
    const closedConfirmed = hasProof && generatedAt >= (lastClosedOpen + tfMs + safetyMs);

    // Điều kiện 3 (khuyến nghị): freshness window để tránh dùng dữ liệu quá cũ nếu API lag/đứt
    // (tùy bạn; nếu không muốn strict thì có thể bỏ freshEnough)
    const freshEnough =
      hasProof && generatedAt < (lastClosedOpen + 2 * tfMs + 30_000); // buffer 30s
    const ind5ts = Number(indicators_ltf?.["5"]?.last?.ts);
    const indicatorAligned = Number.isFinite(ind5ts) && ind5ts === lastClosedOpen;
    const ltfTriggerReady = Boolean(closedConfirmed && freshEnough && indicatorAligned);

    const ltf_trigger_state = {
      tf: "5",
      ready: ltfTriggerReady,
      reason: ltfTriggerReady
        ? "ok"
        : !hasProof
          ? "no_closed_candle_proof"
          : !closedConfirmed
