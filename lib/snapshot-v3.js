...
// Gọi backend /api/bybit/onchain để lấy block onchain
async function fetchOnchainBlock(primaryBaseAsset = "BTC") {
  // Nếu chưa cấu hình backend, trả rỗng để không phá snapshot
  if (!ONCHAIN_API_BASE) {
    console.warn(
      "[snapshot-v3.1] NEXT_PUBLIC_ONCHAIN_API_BASE is not set – onchain will be empty."
    );
    return {
  ...
  try {
    ...
    url.searchParams.set("asset", primaryBaseAsset);
    ...
    if (!res.ok) {
      ...
      console.error(
        "[snapshot-v3.1] fetchOnchainBlock HTTP error:",
        res.status,
        res.statusText,
        text.slice(0, 200)
      );
    ...
    console.error(
      "[snapshot-v3.1] fetchOnchainBlock error:",
      err.message || err
    );
    return {
      exchange_netflow_daily: [],
...
export async function buildSnapshotV3(symbols) {
  ...
  // Xác định asset chính để lấy onchain – base của symbol đầu tiên
  let primaryBaseAsset = "BTC";
  if (symbols && symbols.length) {
    const firstSym = symbols[0];
    if (firstSym.endsWith("USDT")) {
      primaryBaseAsset = firstSym.slice(0, -4);
    } else {
      primaryBaseAsset = firstSym;
    }
  }

  // Lấy onchain từ backend (Dune proxy). Nếu lỗi -> trả rỗng.
  const onchain = await fetchOnchainBlock(primaryBaseAsset);
