...
function markSweepThenAbsorb(fullSnap) {
  ...
  function pickRelevantSweep(sweeps, nowMs, currentPrice) {
    ...
    // Chấm điểm: ưu tiên vừa gần thời gian vừa gần giá
    let best = null;
    let bestScore = -Infinity;
    ...
    for (const s of norm) {
      ...
      const age = nowMs - s.ts_closed;
      if (age < 0 || age > w) continue; // không recent theo TF

      // distance theo % giá hiện tại
      const distPct = Math.abs(s.swept_level - currentPrice) / currentPrice; // 0.01 = 1%
      // Nếu sweep quá xa (ví dụ > 3%), coi như không liên quan LTF entry hiện tại
      if (distPct > 0.03) continue;

      // score: càng mới càng tốt, càng gần giá càng tốt
      const timeScore = 1 - (age / w);           // 0..1
      const distScore = 1 - Math.min(1, distPct / 0.03); // 0..1
      const score = 0.65 * timeScore + 0.35 * distScore;
  ...
  for (const [sym, ltf] of Object.entries(ltfSymbols)) {
    ...
    // === (NEW) Pick "relevant sweep" theo time window + distance to current price ===
    const now = fullSnap?.generated_at || Date.now();

    // Lấy giá hiện tại từ LTF debug nếu có, fallback sang ticker HTF
    const currentPrice =
    ...
    // Nếu không có sweep phù hợp (quá cũ hoặc quá xa giá), flag luôn false
    if (!lastSweep) {
    ...
    // Match hướng evidence với sweep side
    ...
    // pickRelevantSweep đã lọc "recent theo TF" rồi -> không cần recentSweep nữa
    // Nếu LTF đã có sweep_context (tức đã tính context-aware) thì không override
    const alreadyContextAware = Boolean(er?.debug?.sweep_context);
    if (!alreadyContextAware) {
      er.sweep_then_absorb_flag = Boolean(divOK || absorbOK);
    }
