...
// ====================== AUDIT BLOCK (STANDARDIZED, NON-BREAKING) ======================
// Mục tiêu: cho AI đọc nhanh closed-candle proof + LTF gate mà không phải scan toàn JSON.
// Không đổi logic: chỉ tóm tắt lại dữ liệu đã có trong merged snapshot.

function _fmtAuditTs(ts, timeZone = "America/Los_Angeles") {
  const n = Number(ts);
  ...
  try {
    ...
    tzShort = tzParts.find((p) => p.type === "timeZoneName")?.value || null;
  } catch (_) {
    tzShort = null;
  }

  return `${local} ${tzShort || "TZ"} | ${d.toISOString()}`;
}

function buildAuditBlockMerged(merged, symbols, timeZone = "America/Los_Angeles") {
  const out = {
    tz: timeZone,
    generated_at: merged?.generated_at ?? null,
    generated_at_fmt: _fmtAuditTs(merged?.generated_at, timeZone),
    symbols: {},
  };

  const htfSyms = merged?.per_exchange?.bybit?.symbols || [];
  const ltfSyms = merged?.per_exchange_ltf?.bybit?.symbols || {};

  for (const sym of symbols || []) {
    const htf = Array.isArray(htfSyms) ? htfSyms.find((x) => x?.symbol === sym) : null;
    const ltf = ltfSyms?.[sym] || null;

    // HTF proof: last closed ts for H1/H4/D (mở rộng nếu bạn muốn)
    const lastClosedMap = htf?.meta?.last_closed_kline_ts || {};
    const h1 = Number(lastClosedMap?.["60"]) || null;
    const h4 = Number(lastClosedMap?.["240"]) || null;
    const d1 = Number(lastClosedMap?.["D"]) || null;

    // LTF proof
    const cs = ltf?.meta?.candle_status || {};
    const m5 = Number(cs?.["5"]?.last_closed_ts) || null;
    const m15 = Number(cs?.["15"]?.last_closed_ts) || null;

    // LTF gate summary
    const gate = ltf?.ltf_trigger_state || null;

    out.symbols[sym] = {
      htf: {
        h1_last_closed_ts: h1,
        h1_last_closed_fmt: _fmtAuditTs(h1, timeZone),
        h4_last_closed_ts: h4,
        h4_last_closed_fmt: _fmtAuditTs(h4, timeZone),
        d1_last_closed_ts: d1,
        d1_last_closed_fmt: _fmtAuditTs(d1, timeZone),
      },
      ltf: {
        m5_last_closed_ts: m5,
        m5_last_closed_fmt: _fmtAuditTs(m5, timeZone),
        m15_last_closed_ts: m15,
        m15_last_closed_fmt: _fmtAuditTs(m15, timeZone),
      },
      ltf_gate: gate
        ? {
          primary_tf: gate.primary_tf ?? "5",
          state: gate.state ?? null,
          actionable: Boolean(gate.actionable),
          reason_code: gate.reason_code ?? null,
          reason_detail: gate.reason_detail ?? null,
        }
        : null,
    };
  }
...
export async function buildFullSnapshotV3(symbols) {
  ...
  merged.audit_block = buildAuditBlockMerged(merged, symbols, "America/Los_Angeles");
  return merged;
}
