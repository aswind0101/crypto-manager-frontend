...
// ISO UTC (machine + human friendly)
function toIsoUtc(ms) {
  if (!ms || !Number.isFinite(ms)) return null;
  return new Date(ms).toISOString();
}

// Local time (human friendly). Default to America/Los_Angeles
function toLocalTime(ms, tz = "America/Los_Angeles") {
  if (!ms || !Number.isFinite(ms)) return null;
  try {
    ...
    // en-CA gives YYYY-MM-DD, we format it into "YYYY-MM-DD HH:mm:ss"
    ...
    // If Intl/timeZone not available in runtime, fallback
    return new Date(ms).toISOString();
  }
}
...
function buildKlinesCompact(klinesRaw, config) {
  ...
  for (const tf of Object.keys(cfg)) {
    ...
    // Parse + sort ASC toàn bộ rồi lấy N nến gần nhất
    const parsedAll = parseKlinesList(raw);
    const parsed = parsedAll.slice(-n);
...
function buildLtfKlinesCompact(klinesRaw, config) {
  ...
  for (const tf of Object.keys(cfg)) {
    ...
    // 1) Parse + sort ASC toàn bộ (an toàn với mọi thứ tự trả về)
    const parsedAll = parseKlinesList(raw);

    // 2) Lấy N nến GẦN NHẤT theo thời gian (đuôi của ASC)
    const parsed = parsedAll.slice(-n);
...
// ====================== LTF EVIDENCE HELPERS (M5/M15) ======================

// interval ("5"|"15") -> ms
function intervalToMs(tf) {
  if (tf === "5") return 5 * 60_000;
  if (tf === "15") return 15 * 60_000;
  return null;
}

// coi nến "đã đóng" nếu now >= ts + interval_ms
function isCandleClosed(ts, tf) {
  const ms = intervalToMs(tf);
  if (!ms || ts == null) return false;
  return Date.now() >= Number(ts) + ms;
}

// nến: body/wick ratio + pattern (heuristic nhẹ để gating READY/trap)
function computeCandleSignal(c) {
  if (!c) return null;
  ...
  const body_ratio = body / range;
  const upper_wick_ratio = upperWick / range;
  const lower_wick_ratio = lowerWick / range;

  let pattern = "none";
  // Pinbar: body nhỏ, wick lớn
  const isPin = body_ratio <= 0.30 && (upper_wick_ratio >= 0.55 || lower_wick_ratio >= 0.55);
  if (isPin) {
    pattern = upper_wick_ratio > lower_wick_ratio ? "pinbar_bear" : "pinbar_bull";
  }
...
// sweep (liquidity grab) rất nhẹ: so với 9 nến trước
function detectSweepFromCompact(klinesCompactArr) {
  if (!Array.isArray(klinesCompactArr) || klinesCompactArr.length < 10) {
    return { sweep_up: false, sweep_down: false, ref: null };
  }

  const sorted = [...klinesCompactArr].sort((a, b) => Number(a.ts) - Number(b.ts));

  const lookback = sorted.slice(-10, -1);
  const last = sorted[sorted.length - 1];

  const prevHigh = Math.max(...lookback.map(x => x.h));
  const prevLow = Math.min(...lookback.map(x => x.l));

  const sweep_up = last.h > prevHigh && last.c < prevHigh;
  const sweep_down = last.l < prevLow && last.c > prevLow;

  return { sweep_up, sweep_down, ref: { prevHigh, prevLow } };
}

// build evidence cho 1 TF (5/15) dựa vào klines_ltf_compact
function buildLtfEvidence(tf, klinesCompactArr) {
  if (!Array.isArray(klinesCompactArr) || !klinesCompactArr.length) {
    return { tf, last_closed_candle: null, sweep: { sweep_up: false, sweep_down: false, ref: null } };
  }

  // IMPORTANT: không tin thứ tự mảng; sort theo ts để lấy candle mới nhất
  const sorted = [...klinesCompactArr].sort((a, b) => Number(a.ts) - Number(b.ts));
  const last = sorted[sorted.length - 1];

  const closed = isCandleClosed(last.ts, tf);
  const signal = computeCandleSignal(last);
  const sweep = detectSweepFromCompact(sorted);
...
// parent HTF lock: age + stale flag (rule đơn giản)
function buildParentHtfLock(parentHtfGeneratedAt) {
  if (!parentHtfGeneratedAt) {
    return { generated_at: null, age_minutes: null, stale: null };
  }
  const ageMin = Math.max(0, (Date.now() - Number(parentHtfGeneratedAt)) / 60_000);
  // Rule: stale nếu > 6h (bạn đổi tuỳ ý)
  const stale = ageMin > 360;
  return { generated_at: Number(parentHtfGeneratedAt), age_minutes: ageMin, stale };
}
  ...
  for (const sym of symbols) {
    ...
    symbolsData.push({
      ...
      meta: {
        ...
        generated_at_iso: toIsoUtc(generatedAt),
        generated_at_local: toLocalTime(generatedAt),
...
export async function buildLtfSnapshotV3(symbols, parentHtfGeneratedAt = null) {
  const generatedAt = Date.now();
  ...
  for (const sym of symbols) {
    ...
    // 2) Ticker + Orderbook + Recent trades (để build orderflow_summary_ltf)
    ...
    // 2.1) Orderflow summary (tái sử dụng helper buildOrderflowSummary có sẵn)
    const orderflow_summary_ltf = buildOrderflowSummary(orderbook, recent_trades);

    // 2.2) LTF evidence (nến đã đóng + pattern + sweep)
    // dùng klines_ltf_compact vì đã gọn và consistent
    const ltf_evidence = {
      "5": buildLtfEvidence("5", klines_ltf_compact["5"] || []),
      "15": buildLtfEvidence("15", klines_ltf_compact["15"] || []),
    };
    ...
    bybitLtfSymbols[sym] = {
      ...
      // ===== NEW: execution blocks cho canh entry =====
      orderflow_summary_ltf,
      ltf_evidence,
  ...
  const payload = {
    ...
    // ===== NEW: ràng buộc HTF tham chiếu =====
    parent_htf: parentHtfGeneratedAt
      ? {
        generated_at: parentHtfGeneratedAt,
        generated_at_iso: toIsoUtc(parentHtfGeneratedAt),
        generated_at_local: toLocalTime(parentHtfGeneratedAt),
        ...buildParentHtfLock(parentHtfGeneratedAt),
      }
      : null,
