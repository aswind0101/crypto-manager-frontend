...
// ====================== KLINES LIMIT CONFIG ======================
// Khuyến nghị: tăng LTF để đủ dữ liệu check đảo chiều/entry timing
const KLINES_LIMIT_BY_TF = {
  "5": 600,
  "15": 400,
  "60": 500,
  "240": 450,
...
// Chỉ lấy TF cần cho plan H1–H4–Daily + LTF (M5/M15)
// limitOrMap có thể là:
// - number: áp cho tất cả (trừ D vẫn dùng 365)
// - object map: { "5": 600, "15": 400, "60": 500, "240": 450, "D": 365 }
async function getKlinesForV3(symbol, intervals = ["60", "240", "D"], limitOrMap = 500) {
  const klines = {};
  for (const interval of intervals) {
    let perTfLimit;

    if (typeof limitOrMap === "object" && limitOrMap) {
      perTfLimit = Number(limitOrMap[interval]);
      if (!Number.isFinite(perTfLimit)) perTfLimit = interval === "D" ? 365 : 500;
    } else {
      perTfLimit = interval === "D" ? 365 : Number(limitOrMap) || 500;
    }
...
// ====================== LTF REVERSAL SIGNALS (M5/M15) ======================

function candleBody(c) {
  return Math.abs((c.close ?? 0) - (c.open ?? 0));
}
function candleRange(c) {
  return Math.max(0, (c.high ?? 0) - (c.low ?? 0));
}
function upperWick(c) {
  const top = Math.max(c.open ?? 0, c.close ?? 0);
  return Math.max(0, (c.high ?? 0) - top);
}
function lowerWick(c) {
  const bot = Math.min(c.open ?? 0, c.close ?? 0);
  return Math.max(0, bot - (c.low ?? 0));
}
function isBull(c) {
...
function detectEngulfing(prev, last) {
  if (!prev || !last) return null;

  // Bullish engulfing: prev đỏ, last xanh và thân last bao trùm thân prev
  if (
    isBear(prev) &&
    isBull(last) &&
    (last.open ?? 0) <= (prev.close ?? 0) &&
    (last.close ?? 0) >= (prev.open ?? 0)
  ) {
...
function detectPinbar(last) {
  if (!last) return null;
  const r = candleRange(last);
  if (!r) return null;

  const b = candleBody(last);
  const uw = upperWick(last);
  const lw = lowerWick(last);

  // Pinbar/hammer/shooting-star heuristic
  const bodySmall = b / r <= 0.35;
  if (!bodySmall) return null;

  // Hammer / bullish pinbar: lower wick dominates
  if (lw / r >= 0.55 && uw / r <= 0.25) return "hammer_pinbar";

  // Shooting star / bearish pinbar: upper wick dominates
  if (uw / r >= 0.55 && lw / r <= 0.25) return "shooting_star";

  return null;
}

// Swing Failure Pattern (SFP) đơn giản:
// - Bear SFP: last.high phá đỉnh gần nhất (lookback), nhưng last.close đóng lại dưới đỉnh đó
// - Bull SFP: last.low phá đáy gần nhất, nhưng last.close đóng lại trên đáy đó
function detectSfp(closed, lookback = 10) {
  if (!Array.isArray(closed) || closed.length < lookback + 2) return null;

  const last = closed[closed.length - 1];
  const prevSlice = closed.slice(-(lookback + 1), -1);

  const prevHigh = Math.max(...prevSlice.map((c) => c.high ?? -Infinity));
  const prevLow = Math.min(...prevSlice.map((c) => c.low ?? Infinity));

  if ((last.high ?? -Infinity) > prevHigh && (last.close ?? 0) < prevHigh) {
    return "bear_sfp";
  }
  if ((last.low ?? Infinity) < prevLow && (last.close ?? 0) > prevLow) {
    return "bull_sfp";
  }
  return null;
}

function scoreStrength({ pattern, sfp, last }) {
  // scoring gọn để AI/logic downstream dễ dùng
  let s = 0;
  if (pattern === "bullish_engulfing" || pattern === "bearish_engulfing") s += 2;
  if (pattern === "hammer_pinbar" || pattern === "shooting_star") s += 1;
  if (sfp) s += 2;

  // bonus nếu candle “quyết liệt” (body tương đối lớn)
  const r = candleRange(last);
  const b = candleBody(last);
  if (r > 0 && b / r >= 0.55) s += 1;
...
function computeLTFReversalSignals(klinesRaw, orderflow_summary) {
  const out = {
    "5": null,
    "15": null,
    aggregate: {
      bias: "neutral", // "bullish_reversal" | "bearish_reversal" | "neutral"
      confidence: "low",
      notes: [],
    },
  };

  if (!klinesRaw) return out;

  const nowMs = Date.now();

  for (const tf of ["5", "15"]) {
    const raw = klinesRaw[tf];
    if (!raw || !raw.length) continue;
    ...
    const sfp = detectSfp(closed, 10);

    const pattern = engulf || pinbar || null;
    const strength = scoreStrength({ pattern, sfp, last });

    // Optional: gắn thêm hint từ orderflow_summary nếu có (không phụ thuộc schema cứng)
    const mmHint =
      orderflow_summary?.labels?.mm_pressure ||
      orderflow_summary?.mm_pressure ||
      null;

    out[tf] = {
      last_closed_ts: last.ts ?? null,
      pattern,       // bullish_engulfing | bearish_engulfing | hammer_pinbar | shooting_star | null
      sfp,           // bull_sfp | bear_sfp | null
      strength,      // weak | medium | strong
      mm_hint: mmHint,
    };
  }

  // Aggregate logic đơn giản: ưu tiên M15, sau đó M5
  const m15 = out["15"];
  const m5 = out["5"];
  const pick = m15 || m5;

  if (pick?.pattern === "bullish_engulfing" || pick?.pattern === "hammer_pinbar" || pick?.sfp === "bull_sfp") {
    out.aggregate.bias = "bullish_reversal";
    out.aggregate.confidence = pick?.strength === "strong" ? "high" : pick?.strength === "medium" ? "medium" : "low";
    out.aggregate.notes.push("Detected bullish reversal signature on LTF (prefer M15).");
  } else if (pick?.pattern === "bearish_engulfing" || pick?.pattern === "shooting_star" || pick?.sfp === "bear_sfp") {
    out.aggregate.bias = "bearish_reversal";
    out.aggregate.confidence = pick?.strength === "strong" ? "high" : pick?.strength === "medium" ? "medium" : "low";
    out.aggregate.notes.push("Detected bearish reversal signature on LTF (prefer M15).");
  }
...
async function collectSymbolDataV3(symbol) {
  ...
  const klines = await getKlinesForV3(symbol, ["60", "240", "D"], {
    "60": 200,
    "240": 200,
    "D": 260,
  });
...
export async function buildSnapshotV3(symbols) {
  ...
  for (const sym of symbols) {
    ...
    // --- SPEC alias hardening: đảm bảo price_structure.H4.trend_label luôn tồn tại ---
    if (price_structure?.H4 && !price_structure.H4.trend_label) {
      price_structure.H4.trend_label = htf_trend_context?.h4_trend_label ?? null;
    }

    // key levels (daily/prev/weekly/monthly) + range_high/low (từ H1 range)
    const keyLevelsBase = buildKeyLevelsFromDaily(parsedD);

    const rangeHigh = price_structure?.H1?.range?.high ?? null;
    const rangeLow = price_structure?.H1?.range?.low ?? null;
...
export async function buildLtfSnapshotV3(symbols) {
  ...
  for (const sym of symbols) {
    ...
    const klinesLtf = await getKlinesForV3(sym, LTF_INTERVALS_V3, KLINES_LIMIT_BY_TF);
    ...
    // Add reversal signals under indicators_ltf
    indicators_ltf.reversal_signals = computeLTFReversalSignals(klinesLtf, orderflow_summary);
...
// ====================== DASH NORMALIZER + VALIDATOR (v3.3-full) ======================

export function normalizeToV33FullForDash(raw) {
  // Case 1: already full v3.3
  if (
    raw?.schema?.name === "price_analyzer_full_snapshot" &&
    raw?.schema?.version === "3.3-full"
  ) {
    return raw;
  }

  // Case 2: v7 wrapper -> unwrap legacy + rebuild minimal v3.3-full shape
  if (raw?.schema?.name === "price_analyzer_snapshot" && raw?.schema?.version === "7.0") {
    const htf = raw?.legacy?.htf || null;
    const ltf = raw?.legacy?.ltf || null;

    if (!htf || !ltf) {
      throw new Error("SNAPSHOT_NOT_FULL: v7 missing legacy.htf or legacy.ltf");
    }

    // Nếu bạn đã có buildFullSnapshotV3() chạy sẵn thì tốt nhất dùng luôn file đó.
    // Ở đây ta normalize tại runtime (đọc file) để bảo đảm DASH luôn có v3.3-full shape.
    const merged = {
      ...(htf || {}),
      schema: {
        name: "price_analyzer_full_snapshot",
      ...
      version: 3.3,
      generated_at: raw?.meta?.generated_at ?? htf?.generated_at ?? Date.now(),
      per_exchange_ltf: ltf?.per_exchange_ltf || {},
      global_derivatives_ltf: ltf?.global_derivatives_ltf || {},
      _meta: {
        htf_generated_at: htf?.generated_at || null,
        ltf_generated_at: ltf?.generated_at || null,
        source_schema: "v7",
      },
    };

    // đảm bảo root shortcuts theo SPEC (đọc trực tiếp ở top-level)
    const primarySymbol = raw?.meta?.primary_symbol || (raw?.symbols?.[0] ?? null);
    if (primarySymbol) attachSpecRootShortcutsV33Full(merged, primarySymbol);

    return merged;
  }

  throw new Error(
    `UNSUPPORTED_SCHEMA_FOR_DASH: ${raw?.schema?.name || "unknown"}@${raw?.schema?.version || "unknown"}`
  );
}

export function validateV33FullForDash(snapshot, symbol) {
  const errs = [];
  const push = (p) => errs.push(p);

  if (snapshot?.schema?.name !== "price_analyzer_full_snapshot") push("schema.name");
  if (snapshot?.schema?.version !== "3.3-full") push("schema.version");

  // Core blocks required by SPEC (full)
  if (!snapshot?.per_exchange?.bybit?.symbols) push("per_exchange.bybit.symbols");
  if (!snapshot?.per_exchange_ltf?.bybit?.symbols) push("per_exchange_ltf.bybit.symbols");

  // Root shortcuts (SPEC uses these heavily)
  if (!snapshot?.ticker) push("ticker");
  if (!snapshot?.indicators?.["60"]?.last) push('indicators["60"].last');
  if (!snapshot?.indicators?.["240"]?.ema) push('indicators["240"].ema');
  if (!snapshot?.price_structure?.H4?.trend_label) push("price_structure.H4.trend_label");

  // LTF orderflow path (Setup #1/#2/#3)
  if (symbol) {
    const ltfSym = snapshot?.per_exchange_ltf?.bybit?.symbols?.[symbol];
    if (!ltfSym) push(`per_exchange_ltf.bybit.symbols[${symbol}]`);
    else {
      if (!ltfSym?.orderflow_summary?.orderbook) push(`per_exchange_ltf.bybit.symbols[${symbol}].orderflow_summary.orderbook`);
      if (!ltfSym?.orderflow_summary?.recent_trades) push(`per_exchange_ltf.bybit.symbols[${symbol}].orderflow_summary.recent_trades`);
      if (!ltfSym?.indicators_ltf?.["15"]?.last) push(`per_exchange_ltf.bybit.symbols[${symbol}].indicators_ltf["15"].last`);
    }
  }

  if (errs.length) {
    const msg = `DASH_VALIDATE_FAIL: missing ${errs.join(", ")}`;
    const e = new Error(msg);
    e.missing_paths = errs;
    throw e;
  }
