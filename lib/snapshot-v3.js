...
export async function buildFullSnapshotV3(symbols) {
  ...
  // ====================== SPEC ROOT SHORTCUTS (PRIMARY SYMBOL) ======================
  const primarySymbol = symbols[0];

  // HTF symbol object (v3: per_exchange.bybit.symbols là array)
  const htfArr = merged?.per_exchange?.bybit?.symbols;
  const primaryHTF = Array.isArray(htfArr)
    ? (htfArr.find((s) => s?.symbol === primarySymbol) || htfArr[0] || null)
    : null;

  // LTF symbol object (v3-ltf: per_exchange_ltf.bybit.symbols là object map)
  const primaryLTF =
    merged?.per_exchange_ltf?.bybit?.symbols?.[primarySymbol] || null;

  // Root shortcuts theo SPEC (AI sẽ đọc ở top-level)
  merged.ticker = primaryHTF?.ticker || primaryLTF?.ticker || null;
  merged.indicators = primaryHTF?.indicators || null;
  merged.price_structure = primaryHTF?.price_structure || null;
  merged.key_levels = primaryHTF?.key_levels || null;
  merged.derived_metrics = primaryHTF?.derived_metrics || null;

  // orderflow_summary nằm ở LTF (đúng theo SPEC setup #1/#2/#3)
  merged.orderflow_summary = primaryLTF?.orderflow_summary || primaryHTF?.orderflow_summary || null;

  merged.sentiment_labels = primaryHTF?.sentiment_labels || null;

  // (optional) root LTF indicators shortcut nếu bạn muốn tiện debug
  merged.indicators_ltf = primaryLTF?.indicators_ltf || null;
  merged.ticker_ltf = primaryLTF?.ticker || null;
