...
function markSweepThenAbsorb(fullSnap) {
  ...
  for (const [sym, ltf] of Object.entries(ltfSymbols)) {
    ...
    // Lấy sweep gần nhất theo ts_closed
    let lastSweep = null;
    if (sweeps.length) {
      const sorted = sweeps
        .map((x) => ({
        ...
        .filter((x) => x.ts_closed && x.side)
        .sort((a, b) => a.ts_closed - b.ts_closed);

      if (sorted.length) lastSweep = sorted[sorted.length - 1];
    }

    // Nếu không có sweep, flag luôn false
    if (!lastSweep) {
      er.sweep_then_absorb_flag = false;
      continue;
    }

    // recent window theo TF (thực dụng)
    const now = fullSnap?.generated_at || Date.now();
    const tf = ["H1", "H4", "D1"].includes(lastSweep.tf) ? lastSweep.tf : "H1";
    const tfWindowMs =
      tf === "H1" ? 6 * 60 * 60 * 1000 :
        tf === "H4" ? 24 * 60 * 60 * 1000 :
    ...
    const recentSweep = (now - lastSweep.ts_closed) <= tfWindowMs;
