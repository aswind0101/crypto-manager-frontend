...
async function getKlinesForV3(symbol, intervals = ["60", "240", "D"], limit = 500) {
  const klines = {};
  for (const interval of intervals) {
    const perTfLimit = interval === "D" ? 365 : limit;
...
export async function buildLtfSnapshotV3(symbols) {
  ...
  for (const sym of symbols) {
    ...
    const klinesLtf = await getKlinesForV3(sym, LTF_INTERVALS_V3, 500);
