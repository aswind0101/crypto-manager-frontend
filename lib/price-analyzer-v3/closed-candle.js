...
// Return { ok, lastClosedTs, proofPathUsed, reason, missingPaths[] }
function getClosedCandleProofHTF(snapshot, symbol, tf) {
  const missing = [];
...
function getClosedCandleProofLTF(snapshot, symbol, tf) {
  const missing = [];
...
// Validate candle last.ts matches last_closed_ts
function assertIndicatorLastIsClosed(indicatorLast, lastClosedTs) {
