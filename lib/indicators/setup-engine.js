...
function rrToScore(rr1) {
    // Smooth saturation: diminishing returns for large RR
    // RR=1 -> ~0.283, RR=2 -> ~0.486, RR=3 -> ~0.632, RR=5 -> ~0.811
    if (!Number.isFinite(rr1) || rr1 <= 0) return 0;
    const k = 3;
    return clamp01(1 - Math.exp(-rr1 / k));
}

function executionPenaltyFromWarnings(warnings = []) {
    // Penalize execution fragility; reduces false positives with "pretty RR but fragile stop"
...
function hardRejectV2({ type, atr, entry, stop, rr1, penetration_atr }) {
    const reasons = [];

    if (!(Number.isFinite(atr) && atr > 0)) {
        reasons.push("hard_reject_atr_missing");
        return { ok: false, reasons };
    }

    const risk = (Number.isFinite(entry) && Number.isFinite(stop)) ? Math.abs(entry - stop) : null;
    const riskOverAtr = Number.isFinite(risk) ? (risk / atr) : null;

    // 1) Stop too tight: not tradable in practice (noise)
    if (!Number.isFinite(riskOverAtr) || riskOverAtr < 0.30) {
        reasons.push("hard_reject_stop_too_tight");
    }

    // 2) RR minimum by type
    if (!Number.isFinite(rr1)) {
        reasons.push("hard_reject_rr_missing");
    } else {
        if ((type === "reversal_sweep" || type === "breakout") && rr1 < 0.90) {
            reasons.push("hard_reject_rr_too_low");
        }
        if (type === "trend_continuation" && rr1 < 0.60) {
            reasons.push("hard_reject_rr_too_low");
        }
    }

    // 3) Reversal sweep must be a real sweep
    if (type === "reversal_sweep") {
        if (Number.isFinite(penetration_atr) && penetration_atr < 0.06) {
            reasons.push("hard_reject_sweep_too_weak");
        }
        if (!Number.isFinite(penetration_atr)) {
            // If missing, treat as not reliable enough for reversal_sweep
            reasons.push("hard_reject_sweep_strength_missing");
        }
    }
...
function isTradableSetupV2(s) {
    if (!s) return false;

    const type = s.type;
    const em = s.execution_metrics || {};
    const riskOverAtr = em.risk_over_atr;
    const rr1 = em.rr_tp1 ?? (s.r_multiple?.tp1 ?? null);

    // Hard floors (mirror hardRejectV2)
    if (!Number.isFinite(riskOverAtr) || riskOverAtr < 0.30) return false;

    if (!Number.isFinite(rr1)) return false;
    if ((type === "reversal_sweep" || type === "breakout") && rr1 < 0.90) return false;
    if (type === "trend_continuation" && rr1 < 0.60) return false;

    // Reversal sweep needs penetration_atr (must be real sweep)
    if (type === "reversal_sweep") {
        const penAtr =
            s?.execution_anchors?.sweep?.quality?.penetration_atr ??
            null;

        if (!Number.isFinite(penAtr) || penAtr < 0.06) return false;
    }

    // Must not be a failed gate (safety)
    const gates = s?.scores?.gates;
    if (gates) {
        for (const k of Object.keys(gates)) {
            if (gates[k] && gates[k].ok === false) return false;
        }
    }
...
// ==========================
// v2 Canonical Normalization + Execution-aware ranking
// - Ensures root mirrors exist (final_score/quality_tier/warnings) from scores.*
// - Adds deterministic execution-priority tie-breakers to prevent "late/chase" primaries
// ==========================

function normalizeSetupMirrorsV2(s) {
    if (!s) return s;

    const sc = s.scores || null;
    if (sc) {
        // Canonical mirrors (scores is source of truth)
        if (Number.isFinite(sc.final_score)) s.final_score = sc.final_score;
        if (sc.quality_tier != null) s.quality_tier = sc.quality_tier;
        if (Array.isArray(sc.warnings)) s.warnings = sc.warnings;

        // Helpful mirrors (optional but consistent)
        if (Number.isFinite(sc.execution_quality) && !Number.isFinite(s.parameter_reliability)) {
            s.parameter_reliability = sc.execution_quality;
        }
        if (Number.isFinite(sc.signal_quality) && !Number.isFinite(s.idea_confidence)) {
            s.idea_confidence = sc.signal_quality;
        }
    }

    // Backstops (never invent values; only fill nulls)
    if (!Number.isFinite(s.final_score)) s.final_score = Number.isFinite(sc?.final_score) ? sc.final_score : 0;
    if (!Array.isArray(s.warnings)) s.warnings = Array.isArray(sc?.warnings) ? sc.warnings : [];

    return s;
}

// Execution priority rank (lower is better)
// Goal: avoid recommending missed/invalidated; prefer ready LIMIT; then ready MARKET; then waiting.
function executionRankV2(s) {
    const es = s?.execution_state || null;
    const phase = String(es?.phase || "waiting");
    const readiness = String(es?.readiness || "no_trade");

    if (phase === "invalidated") return 90;
    if (phase === "missed") return 80;

    // "ready" buckets
    if (phase === "ready" && readiness === "limit_ok") return 10;
    if (phase === "ready" && readiness === "market_ok") return 20;

    // "active" (if you later add it)
    if (phase === "active") return 25;

    // waiting
    return 40;
}

function sortSetupsV2(a, b) {
    // 1) Execution readiness first (prevents "late" primaries)
    const ea = executionRankV2(a);
    const eb = executionRankV2(b);
    ...
    // 3) Reliability tie-breaker
    const pa = nz(a?.parameter_reliability, 0);
    const pb = nz(b?.parameter_reliability, 0);
    return pb - pa;
}

function pickPrimaryAlternativeV2(tradable, scoreFloor = 0.50) {
    const notes = [];
    if (!Array.isArray(tradable) || tradable.length === 0) {
        notes.push("no_tradable_setups");
        return { primary: null, alternative: null, notes };
    }

    // Prefer "recommendable"
    const recommendable = tradable.filter(s => nz(s?.final_score, 0) >= scoreFloor);

    const pool = (recommendable.length > 0) ? recommendable : tradable;

    // Sort by execution readiness then by score
    const ordered = pool.slice().sort(sortSetupsV2);

    const primary = ordered[0] || null;

    // Alternative = next best different instance (not necessarily different type)
    let alternative = null;
    for (let i = 1; i < ordered.length; i++) {
        if (ordered[i] && ordered[i] !== primary) { alternative = ordered[i]; break; }
    }

    // Notes per canonical
    if (primary && nz(primary.final_score, 0) < scoreFloor) {
        notes.push("primary_below_score_floor_used_best_tradable");
    }

    return { primary, alternative, notes };
}

// ==========================
// Execution State v2 (actionability layer)
// - Determines: ready/waiting/missed/invalidated + MARKET vs LIMIT
// - Conservative: avoids chase and late entries
// ==========================

function getRefPxV2(snapshot, preferEx = "bybit", preferField = "mark") {
    // Prefer mark price -> last -> index. Fallback to unified price if exists.
...
function annotateExecutionStateV2(setup, snapshot, refPx) {
    ...
    const rr1 = nz(setup?.execution_metrics?.rr_tp1, null);

    const tradable = isTradableSetupV2(setup);

    // Safety: if we cannot even reference price, do not suggest execution
    const reasons = [];
    ...
    const distToEntryAtr =
        (Number.isFinite(distToEntry) && Number.isFinite(atr) && atr > 0) ? (distToEntry / atr) : null;

    const distToZoneAtr =
        (Number.isFinite(dz) && Number.isFinite(atr) && atr > 0) ? (dz / atr) : null;

    // Invalidation awareness (hard safety; but can only check directionally)
    // For Long: invalidated if ref <= stop (rough, but safe)
    // For Short: invalidated if ref >= stop
    let invalidated = false;
    if (Number.isFinite(refPx) && Number.isFinite(stop) && side) {
    ...
    // Trigger awareness by type (conservative)
    let triggered = false;
    if (type === "reversal_sweep") {
        const ts = nz(setup?.execution_anchors?.sweep?.ts, null);
        const cc = nz(setup?.execution_anchors?.sweep?.confirm_close, null);
        triggered = Number.isFinite(cc) && Number.isFinite(ts);
        if (!triggered) reasons.push("waiting_reversal_confirmation");
    } else if (type === "breakout") {
        // Breakout is "triggered" once price is meaningfully beyond the BOS band in the direction,
        // but we remain conservative: prefer retest LIMIT in zone, and avoid chasing.
        // If no refPx, cannot decide.
        triggered = Number.isFinite(refPx); // we can evaluate state; actual action still guarded below
    } else if (type === "trend_continuation") {
        // Continuation: prefer pullback into EMA band (zone). Trigger = being in/near zone.
        triggered = inside || (Number.isFinite(distToZoneAtr) && distToZoneAtr <= 0.10);
    }

    // Chase guards (key to avoid late/incorrect execution)
    // - If outside zone by too much, mark as "missed" even if idea is tradable.
    // Thresholds (ATR-based, stable across symbols):
    // - MARKET allowed only if very close to preferred entry: <= 0.10 ATR
    // - "Missed" if moved away from zone: >= 0.25 ATR (reversal/trend), >= 0.30 ATR (breakout can run more)
    const marketMaxDistAtr = 0.10;
    const missedDistAtr = (type === "breakout") ? 0.30 : 0.25;
    ...
    // If not tradable at all, never recommend execution
    if (!tradable) {
        phase = "waiting";
        readiness = "no_trade";
        ...
        // Decide "missed"
        if (Number.isFinite(distToZoneAtr) && distToZoneAtr >= missedDistAtr) {
            phase = "missed";
            readiness = "no_trade";
            reasons.push("price_too_far_from_entry_zone");
        } else {
            // Determine ready/active:
            // - ready: inside zone and (triggered if type needs it)
            // - active: inside zone and triggered OR near zone and very close to entry
            const canEnterNow =
                inside && (type !== "reversal_sweep" ? true : triggered);

            if (canEnterNow) {
                phase = "ready";
                readiness = "limit_ok";
                orderType = "LIMIT";
                orderPrice = Number.isFinite(ep) ? ep : refPx;
                tif = "GTC";
                reasons.push("inside_entry_zone");
                if (type === "reversal_sweep") reasons.push("reversal_confirmed");
            } else {
                // Not inside zone: allow MARKET only if extremely close to entry (avoid chasing),
                // AND idea is already in "triggered context" (for reversal: confirmed; for others: ok).
                const allowMarket =
                    (Number.isFinite(distToEntryAtr) && distToEntryAtr <= marketMaxDistAtr) &&
                    (type === "reversal_sweep" ? triggered : true);
                ...
                if (allowMarket) {
                    ...
                    phase = "waiting";
                    readiness = "no_trade";
                    if (!inside) reasons.push("waiting_pullback_to_zone");
                    if (type === "reversal_sweep" && !triggered) reasons.push("waiting_trigger");
                }
            }
        }
    }

    // Additional explanatory flags (non-blocking)
    if (Number.isFinite(rr1) && rr1 < 1.0) reasons.push("rr_is_modest");
    if (Number.isFinite(distToEntryAtr) && distToEntryAtr > 0.12) reasons.push("avoid_chasing");
...
function computeParameterReliability({
    ...
    frictionPenalty = 0,
    anchor_kind = "sweep",
    anchor_strength_atr = null,
}) {
    let pr = 0;
    const warnings = [];

    // 1) Anchor clarity (0..0.30)
    let a = 0;

    const hasPrimaryAnchor =
        (anchor_kind === "sweep" && hasSweep) ||
        (anchor_kind === "level") ||
        (anchor_kind === "ema");

    if (hasPrimaryAnchor) {
        a += (anchor_kind === "sweep") ? 0.18 : (anchor_kind === "level" ? 0.16 : 0.12);
    } else {
        warnings.push("anchor_missing");
    }

    if (Number.isFinite(anchorTs) && Number.isFinite(nowTs)) {
        const ageMs = nowTs - anchorTs;
        if (ageMs >= 0 && ageMs <= 12 * 60 * 60 * 1000) a += 0.07;
        else if (ageMs <= 48 * 60 * 60 * 1000) a += 0.04;
        else warnings.push("anchor_old");
    } else {
        warnings.push("anchor_ts_missing");
    }

    // Strength: sweep uses penetration; level/ema can pass anchor_strength_atr
    let strengthAtr = null;
    if (anchor_kind === "sweep" && Number.isFinite(sweepQuality?.penetration)) {
        strengthAtr = sweepQuality.penetration / atr;
    } else if (anchor_kind !== "sweep" && Number.isFinite(anchor_strength_atr)) {
        strengthAtr = anchor_strength_atr;
    }

    if (Number.isFinite(strengthAtr)) {
        if (strengthAtr >= 0.12) a += 0.05;
        else warnings.push("anchor_weak");
    } else {
        // not fatal for level/ema, but reduce PR transparency
...
function buildReversalSweepV2({ snapshot, tf = "60" }) {
    ...
    const rr1 = rr(entryPreferred, stopR, tp1R) ?? 0;
    const penetrationAtr =
        (Number.isFinite(atr) && atr > 0 && Number.isFinite(sweepLast.quality?.penetration))
            ? (sweepLast.quality.penetration / atr)
            : null;

    const hr = hardRejectV2({
        type: "reversal_sweep",
        atr,
        entry: entryPreferred,
        stop: stopR,
        rr1,
        penetration_atr: penetrationAtr,
    });

    if (!hr.ok) return null;

    const rrScore = rrToScore(rr1);
    const execPenalty = executionPenaltyFromWarnings(prRes.warnings);
...
function gateResult(ok, reasons = [], penalty = 0) {
    return { ok: !!ok, reasons, penalty: clamp(nz(penalty, 0), 0, 0.25) };
}

function breakoutFakeoutGate({
    side,                 // "long" | "short"
    entryPreferred,
    entryZone,
    stop,
    atr,
    bosLast,              // primitive: {price, ts, side}
    tfBosRaw,             // raw: {i,type,level} optional
    anchorLayer,
    tp1,         // for nowTs
}) {
    const reasons = [];
    ...
    if (!(Number.isFinite(atr) && atr > 0)) return gateResult(false, ["atr_missing"]);
    if (!bosLast || !Number.isFinite(bosLast.price)) return gateResult(false, ["bos_missing"]);

    // Risk sanity
    const risk = (Number.isFinite(entryPreferred) && Number.isFinite(stop)) ? Math.abs(entryPreferred - stop) : null;
    if (!Number.isFinite(risk) || risk <= 0) return gateResult(false, ["risk_missing"]);

    const rAtr = risk / atr;
    if (rAtr < 0.5) return gateResult(false, ["stop_too_tight"]);
    if (rAtr > 2.2) return gateResult(false, ["stop_too_wide"]);
    // Minimum RR gate for breakout: if tp1 exists and RR is too low, reject
    if (Number.isFinite(tp1) && Number.isFinite(entryPreferred) && Number.isFinite(stop)) {
        const rr1 = rr(entryPreferred, stop, tp1);
        if (Number.isFinite(rr1) && rr1 < 0.9) {
            return gateResult(false, ["breakout_rr_too_low"]);
        }
    } else {
        // if we don't have tp1 level, breakout is not actionable
        return gateResult(false, ["tp1_missing_level"]);
    }

    // TP must be beyond BOS level by a small margin (avoid tp sitting on the level)
    const lvl = bosLast.price;
    const margin = 0.10 * atr; // safe default
    if (side === "long" && tp1 <= (lvl + margin)) return gateResult(false, ["tp1_not_beyond_bos"]);
    if (side === "short" && tp1 >= (lvl - margin)) return gateResult(false, ["tp1_not_beyond_bos"]);

    // Retest band sanity: entry zone should straddle BOS level
    //const lvl = bosLast.price;
    if (Array.isArray(entryZone) && entryZone.length === 2) {
        const lo = Math.min(entryZone[0], entryZone[1]);
        const hi = Math.max(entryZone[0], entryZone[1]);
        if (!(lo <= lvl && hi >= lvl)) {
            // Not fatal, but indicates it's not a retest design
            reasons.push("no_bos_retest_band");
            penalty += 0.06;
        }
    }

    // Freshness: BOS too old is prone to being "used up"
    const nowTs = anchorLayer?.market?.now_ts ?? Date.now();
    const anchorTs = bosLast.ts ?? null;
    if (Number.isFinite(anchorTs)) {
        const ageMs = nowTs - anchorTs;
        // tighten by TF-agnostic threshold (safe default)
        if (ageMs > 5 * 24 * 60 * 60 * 1000) {
            reasons.push("bos_old");
            penalty += 0.08;
        }
    } else {
        reasons.push("bos_ts_missing");
        penalty += 0.04;
    }

    // Minimal fakeout heuristic (no raw candles yet):
    // If BOS raw exists but side mismatch (data inconsistency), reject.
    if (tfBosRaw && tfBosRaw.type) {
        const rawSide = tfBosRaw.type === "BOS_UP" ? "long" : (tfBosRaw.type === "BOS_DOWN" ? "short" : null);
        if (rawSide && rawSide !== side) return gateResult(false, ["bos_side_mismatch"]);
    }

    return gateResult(true, reasons, penalty);
}

function trendStrengthGate({
    side,                 // "long" | "short"
    tfFeat,               // snapshot.unified.features.timeframes[tf]
    reg,                  // scoreRegime output
    atr,
    atrPct,
    rr1 = null,
}) {
    ...
    if (!tfFeat?.last) return gateResult(false, ["tf_features_missing"]);
    if (!(Number.isFinite(atr) && atr > 0)) return gateResult(false, ["atr_missing"]);

    const trendLabel = tfFeat?.labels?.trend || "range";
    if (trendLabel === "range") return gateResult(false, ["trend_range"]);

    // EMA alignment gate (low-risk, high-impact)
    const e20 = tfFeat.last.ema20, e50 = tfFeat.last.ema50, e100 = tfFeat.last.ema100, e200 = tfFeat.last.ema200;
    const hasE = [e20, e50, e100, e200].every(Number.isFinite);

    if (hasE) {
        const bull = e20 > e50 && e50 > e100 && e100 > e200;
        const bear = e20 < e50 && e50 < e100 && e100 < e200;

        if (side === "long" && !bull) { reasons.push("ema_stack_not_bull"); penalty += 0.10; }
        if (side === "short" && !bear) { reasons.push("ema_stack_not_bear"); penalty += 0.10; }
    } else {
    ...
    // RSI bias gate (soft)
    const rsi = tfFeat.last.rsi14;
    if (Number.isFinite(rsi)) {
        if (side === "long" && rsi < 45) { reasons.push("rsi_weak_for_long"); penalty += 0.06; }
        if (side === "short" && rsi > 55) { reasons.push("rsi_weak_for_short"); penalty += 0.06; }
    } else {
        reasons.push("rsi_missing");
        penalty += 0.03;
    }

    // Volatility regime: too low ATR% tends to chop (soft)
    if (Number.isFinite(atrPct)) {
        if (atrPct < 0.003) { reasons.push("atr_pct_too_low"); penalty += 0.05; }
        if (atrPct > 0.030) { reasons.push("atr_pct_too_high"); penalty += 0.05; }
    }

    // HTF regime conflict (soft): if composite strongly opposes side, reject
    if (Number.isFinite(reg?.composite)) {
        if (side === "long" && reg.composite < -0.6) return gateResult(false, ["htf_regime_conflict"]);
        if (side === "short" && reg.composite > 0.6) return gateResult(false, ["htf_regime_conflict"]);
    }
    if (Number.isFinite(rr1) && rr1 < 1.0) {
        reasons.push("rr_low_for_continuation");
        penalty += 0.06;
    }
    // If penalties stack too high, reject (prevents "weak continuation" spam)
    if (penalty >= 0.22) return gateResult(false, ["trend_strength_insufficient"].concat(reasons));

    return gateResult(true, reasons, penalty);
}


function buildBreakoutV2({ snapshot, tf = "60" }) {
    const anchorLayer = snapshot?.unified?.anchor_layer;
    if (!anchorLayer) return null;
    ...
    const scores = snapshot?.unified?.scores || {};
    const baseOverall = 0.50 + 0.20 * (clamp(nz(scores.overall, 0.55), 0, 1) - 0.50);
    ...
    const { vol: atr, swingsRaw, bosLast, swingLast } = pickTfAnchor(anchorLayer, tf);
    if (!Number.isFinite(atr) || atr <= 0) return null;
    if (!bosLast || !Number.isFinite(bosLast.price)) return null;

    const step = chooseStep({ tickSize, atr, minStep: 0.1, atrFactor: 0.01 });

    // Direction from BOS side
    ...
    // Stop: prefer opposite swing if available; else ATR buffer beyond level
    const stopBuf = 0.35 * atr;
    let stop = null;
    if (side === "long" && swingLast?.type === "low" && Number.isFinite(swingLast.price)) {
        stop = swingLast.price - stopBuf;
    } else if (side === "short" && swingLast?.type === "high" && Number.isFinite(swingLast.price)) {
        stop = swingLast.price + stopBuf;
    } else {
        stop = side === "long" ? (level - 0.90 * atr) : (level + 0.90 * atr);
    }
    ...
    // ---- Quality gate: breakout fakeout filter ----
    const gate = breakoutFakeoutGate({
        side,
        entryPreferred,
        entryZone,
        stop,
        ...
        atr,
        bosLast,
        tfBosRaw: anchorLayer?.structure?.by_tf?.[tf]?.bos_last_raw ?? null,
        anchorLayer,
    });
    ...
    const frictionPenalty = (() => {
        ...
        if (Number.isFinite(ofC) && ofC < 0.45) fp += 0.06;
        if (liqFeat?.observed && nz(liqFeat.intensity_15m, 0) > 0.15) fp += 0.04;
        return clamp(fp + gate.penalty, 0, 0.20);
    })();

    // Anchor strength proxy: assume level anchor “medium” if BOS exists; slightly higher if ATR% is low (cleaner)
    const atrPct = anchorLayer?.volatility?.atr_pct?.[tf];
    const anchorStrengthAtr =
        Number.isFinite(atrPct) && atrPct < 0.01 ? 0.16 : 0.12;
    ...
    const prRes = computeParameterReliability({
        ...
        sweepQuality: null,
        nowTs,
        anchorTs: bosLast.ts ?? null,
        frictionPenalty,
        anchor_kind: "level",
        anchor_strength_atr: anchorStrengthAtr,
    });

    const pr = prRes.parameter_reliability;

    const rr1 = rr(entryPreferred, stop, tp1R) ?? 0;
    const hr = hardRejectV2({
        type: "breakout",
        atr,
        entry: entryPreferred,
        stop,
        rr1,
        penetration_atr: null,
    });

    if (!hr.ok) return null;
    const rrScore = rrToScore(rr1);
    const execPenalty = executionPenaltyFromWarnings(prRes.warnings.concat(gate.reasons || []));
    ...
    const trigger = side === "long"
        ? "BOS_UP confirmed → retest band around BOS level → continuation breakout long"
        : "BOS_DOWN confirmed → retest band around BOS level → continuation breakout short";
    ...
    const setup = {
        ...
        targets: { tp1: tp1R, tp2: tp2R, tp3: null },
        r_multiple: {
            tp1: rr(entryPreferred, stop, tp1R),
            tp2: rr(entryPreferred, stop, tp2R),
            tp3: null,
        },

        // New standardized metrics blocks
        execution_metrics: {
            atr,
            entry_zone_width: Math.abs(entryZone[1] - entryZone[0]),
            stop_distance: Number.isFinite(entryPreferred) ? Math.abs(entryPreferred - stop) : null,
            risk_over_atr: (Number.isFinite(entryPreferred) && Number.isFinite(atr) && atr > 0)
                ? Math.abs(entryPreferred - stop) / atr
                : null,
            reward_tp1: Number.isFinite(tp1R) ? Math.abs(tp1R - entryPreferred) : null,
            rr_tp1: rr(entryPreferred, stop, tp1R),
        },

        execution_anchors: {
            level: { kind: "BOS", side: bosLast.side, price: level, ts: bosLast.ts ?? null },
            stop_anchor: (side === "long") ? "swing_low+atr_buffer OR level-atr" : "swing_high+atr_buffer OR level+atr",
            tp_anchors: ["nearest_opposing_swing_levels"],
        },

        signal_metrics: {
            regime: { label: reg?.regime ?? null, composite: reg?.composite ?? null, strength: reg?.strength ?? null },
            orderflow: ofAlign.usable ? { score: ofAlign.score, confidence: ofBest?.confidence ?? null } : null,
            derivatives: { penalty: derivPen, funding_labels: deriv?.funding_labels ?? null },
            liquidations: liqSig.usable ? { score: liqSig.score, intensity_15m: liqFeat?.intensity_15m ?? null } : null,
        },
        ...
        scores: {
            ...
            quality_tier: qualityTier(finalScore),
            gates: {
                breakout_fakeout: { ok: gate.ok, reasons: gate.reasons, penalty: gate.penalty },
            },
            warnings: prRes.warnings.concat(gate.reasons || []),
        },
...
function buildTrendContinuationV2({ snapshot, tf = "60" }) {
    const anchorLayer = snapshot?.unified?.anchor_layer;
    if (!anchorLayer) return null;
    ...
    const { vol: atr, swingsRaw, swingLast } = pickTfAnchor(anchorLayer, tf);
    if (!Number.isFinite(atr) || atr <= 0) return null;
    ...
    const ema20 = tfFeat.last.ema20;
    const ema50 = tfFeat.last.ema50;
    if (!Number.isFinite(ema20) || !Number.isFinite(ema50)) return null;

    const step = chooseStep({ tickSize, atr, minStep: 0.1, atrFactor: 0.01 });

    // Entry: EMA band pullback zone
    const lo = Math.min(ema20, ema50);
    const hi = Math.max(ema20, ema50);
    const w = Math.max(0.06 * atr, 0.05 * atr);

    let entryZone = [lo - w, hi + w];
    let entryPreferred = side === "long"
        ? (lo + 0.60 * (hi - lo)) // closer to ema20 in bull trend
        : (hi - 0.60 * (hi - lo)); // closer to ema20 in bear trend (mirror)

    // Stop: prefer recent swing in trend direction invalidation
    const stopBuf = 0.35 * atr;
    let stop = null;
    if (side === "long" && swingLast?.type === "low" && Number.isFinite(swingLast.price)) {
        stop = swingLast.price - stopBuf;
        ...
        stop = side === "long" ? (entryZone[0] - 0.60 * atr) : (entryZone[1] + 0.60 * atr);
    }
    ...
    // ---- Quality gate: trend strength filter ----
    const atrPct = anchorLayer?.volatility?.atr_pct?.[tf] ?? null;

    const gate = trendStrengthGate({
        side,
        tfFeat,
        reg,
        atr,
    ...
    // EMA anchor strength proxy: tighter ATR% => cleaner continuation
    //const atrPct = anchorLayer?.volatility?.atr_pct?.[tf];
    const anchorStrengthAtr =
        Number.isFinite(atrPct) && atrPct < 0.01 ? 0.14 : 0.11;
    ...
    const prRes = computeParameterReliability({
        ...
        sweepQuality: null,
        nowTs,
        anchorTs: anchorLayer?.market?.now_ts ?? null,
        frictionPenalty,
        anchor_kind: "ema",
        anchor_strength_atr: anchorStrengthAtr,
    });
    ...
    const rr1b = rr(entryPreferred, stop, tp1R) ?? 0;
    const hr = hardRejectV2({
        type: "trend_continuation",
        atr,
        entry: entryPreferred,
    ...
    const trigger = side === "long"
        ? "Bull trend (EMA stack) → pullback into EMA band → trend continuation long"
        : "Bear trend (EMA stack) → pullback into EMA band → trend continuation short";
    ...
    const setup = {
        ...
        execution_anchors: {
            ...
            stop_anchor: (side === "long") ? "swing_low+atr_buffer OR ema_band-atr" : "swing_high+atr_buffer OR ema_band+atr",
            tp_anchors: ["nearest_opposing_swing_levels"],
        },
        ...
        scores: {
            ...
            quality_tier: qualityTier(finalScore),
            gates: {
                trend_strength: { ok: gate.ok, reasons: gate.reasons, penalty: gate.penalty },
            },
            warnings: prRes.warnings.concat(gate.reasons || []),
        },
...
export function buildSetupsV2(snapshot, opts = {}) {
    ...
    for (const tf of tfs) {
        ...
        if (rs?.setup) { candidates.push(rs.setup); regOut = regOut || rs.reg; }

        const bo = buildBreakoutV2({ snapshot, tf });
        if (bo?.setup) { candidates.push(bo.setup); regOut = regOut || bo.reg; }

        const tc = buildTrendContinuationV2({ snapshot, tf });
        if (tc?.setup) { candidates.push(tc.setup); regOut = regOut || tc.reg; }
    }

    // ---- NEW: attach execution_state before ranking ----
    const refPx = getRefPxV2(snapshot, "bybit", "mark");
    for (let i = 0; i < candidates.length; i++) {
        candidates[i] = annotateExecutionStateV2(candidates[i], snapshot, refPx);
    }

    // ---- NEW: normalize mirrors (scores.* is canonical) ----
    for (let i = 0; i < candidates.length; i++) {
        candidates[i] = normalizeSetupMirrorsV2(candidates[i]);
    }

    // ---- NEW: attach execution_state if the function exists ----
    if (typeof annotateExecutionStateV2 === "function") {
        const refPx = (typeof getRefPxV2 === "function")
            ? getRefPxV2(snapshot, "bybit", "mark")
            : null;
    ...
    // ---- Sort candidates with execution-aware tiebreakers ----
    candidates.sort(sortSetupsV2);

    const tradable = candidates.filter(isTradableSetupV2);

    // ---- Canonical primary selection: recommendable first, else best tradable ----
    const pick = pickPrimaryAlternativeV2(tradable, 0.50);
    const primary = pick.primary;
    const alternative = pick.alternative;

    const notes = [];
    // Merge notes from picker (canonical)
    for (const n of (pick.notes || [])) notes.push(n);
    ...
    return {
        ...
        primary,
        alternative,
        top_candidates: tradable.slice(0, 6),
        notes,
    };
}
