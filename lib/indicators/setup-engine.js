...
// ==========================
// Setup Visibility Layer (v2)
// Expose ALL setups (tradable / waiting / rejected / unavailable) for UI watchlist
// ==========================
...
function markUnavailable({ symbol, type, tf, reason }) {
    const s = {
        symbol,
        type,
        bias: null,
        ...
        trigger: null,
        entry_zone: null,
        entry_preferred: null,
        invalidation: null,
        stop: null,
        targets: { tp1: null, tp2: null, tp3: null },
        r_multiple: { tp1: null, tp2: null, tp3: null },
        idea_confidence: 0,
        ...
        execution_metrics: {
            atr: null,
            entry_zone_width: null,
            stop_distance: null,
            risk_over_atr: null,
            reward_tp1: null,
            rr_tp1: null,
        },
        execution_anchors: null,
        signal_metrics: null,
        scores: {
            execution_quality: 0,
            signal_quality: 0,
            ...
            quality_tier: "D",
            gates: {},
            hard_reject: { ok: false, reasons: [reason || "unavailable"] },
            warnings: [reason || "unavailable"],
        },
        execution_state: {
            asof_ts: Date.now(),
            ...
            phase: "waiting",
            readiness: "no_trade",
            order: { type: null, price: null, tif: null },
            proximity: { dist_to_entry: null, dist_to_entry_atr: null, inside_entry_zone: false },
            reason: ["not_tradable", reason || "unavailable"],
        },
    };
    return setEligibility(s, { tradable: false, status: "unavailable", reasons: [reason || "unavailable"] });
}

function markRejected(setup, reasons = [], gateName = null, gateObj = null, hardRejectObj = null) {
    const rs = Array.from(new Set((reasons || []).map(String))).filter(Boolean);

    setup.warnings = Array.from(new Set([...(setup.warnings || []), ...rs]));
    setup.scores = setup.scores || {};
    setup.scores.warnings = Array.from(new Set([...(setup.scores.warnings || []), ...rs]));
    setup.scores.final_score = 0;
    setup.final_score = 0;
    setup.quality_tier = "D";

    if (gateName) {
        setup.scores.gates = setup.scores.gates || {};
        setup.scores.gates[gateName] = gateObj || { ok: false, reasons: rs, penalty: 0 };
    }
    if (hardRejectObj) {
        setup.scores.hard_reject = hardRejectObj;
    }

    return setEligibility(setup, { tradable: false, status: "rejected", reasons: rs });
}
...
export function buildSetupsV2(snapshot, opts = {}) {
    ...
    const alternative = tradable[1] || null;
