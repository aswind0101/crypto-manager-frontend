...
// Setup Engine v1.1 (drop-in, based on your v1.0 file)
// Upgrades:
// - Liquidity grab polarity alignment (fixes long/short reversal symmetry bug)
// - Reversal sweep entry/zone anchored to swing + ATR buffer
// - Confidence aggregation: logistic + cap + evidenceStrong unlock (prevents confidence=1 saturation)
...
function orderflowAlignment(orderflow, side) {
  ...
  return { usable, score: clamp((score + 1) / 2, 0, 1) };
}
...
function computeEntryStopFromEMA(tf, side) {
  const l = tf?.last || {};
  const close = nz(l.close ?? null, null); // optional (if you add it in snapshot)
...
// v1.1: polarity for liquidity grab
function grabPolarity(grab) {
  const t = String(grab?.type || "");
  if (t.includes("grab_down") || t.includes("reclaim")) return 1;     // bullish reversal
  if (t.includes("grab_up") || t.includes("reject")) return -1;       // bearish reversal
  return 0;
}

// v1.1: confidence aggregation (logistic) + cap + optional unlock
function combineConfidence(baseConfidence, modifiers, evidenceStrong) {
  ...
  return evidenceStrong ? Math.min(conf, 0.97) : Math.min(conf, 0.90);
}
...
export function buildSetupsV1(snapshot) {
  ...
  function genForSide(side) {
    ...
    // 4) Reversal sweep (polarity-aware + swing-anchored entry)
    {
      const rationale = [];
      let modifier = 0;

      const hasGrab = !!grab?.type;
      const pol = gPol; // +1 bullish, -1 bearish
  ...
  // v1.1: alternative prefers different TYPE first, then different BIAS
