...
// ------------------------------
// NEW: Headline market position (mixed regime, retail wording)
// ------------------------------
function buildMarketPositionVN(regimeLabel, trends) {
  const t15 = trends?.t15 || null;
  const t60 = trends?.t60 || null;
  const t240 = trends?.t240 || null;
  const tD = trends?.tD || null;

  // If regime explicitly says range/sideways
  if (regimeLabel === "range" || regimeLabel === "sideways") {
    // check if HTF and LTF conflict -> emphasize tug-of-war
    if ((t60 === "bull" && tD === "bear") || (t60 === "bear" && tD === "bull")) {
      return "Ngắn hạn và khung ngày đang ngược chiều; tổng thể giằng co (range).";
    }
    return "Thị trường đang đi ngang (range), dễ quét 2 đầu.";
  }

  // Mixed: 1H vs 1D conflict
  if ((t60 === "bull" && tD === "bear") || (t60 === "bear" && tD === "bull")) {
    return `Ngắn hạn đang ${trendLabelToVN(t60).tone}, nhưng khung ngày ${trendLabelToVN(tD).tone}; tổng thể giằng co.`;
  }

  // Agreement on higher timeframes
  if (tD === "bull" || tD === "bear") {
    const d = trendLabelToVN(tD);
    // Mention intraday if different
    if (t60 && t60 !== tD && (t60 === "bull" || t60 === "bear")) {
      return `Xu hướng chính ${d.tone}, ngắn hạn ${trendLabelToVN(t60).tone}.`;
    }
    return `Xu hướng chính đang ${d.tone}.`;
  }

  // Fallback: majority among TFs
  const labels = [t15, t60, t240, tD].filter(Boolean);
  const bull = labels.filter((x) => x === "bull").length;
  const bear = labels.filter((x) => x === "bear").length;
  if (bull > bear && bull >= 2) return "Thị trường nghiêng tăng (nhiều khung đồng thuận).";
  if (bear > bull && bear >= 2) return "Thị trường nghiêng giảm (nhiều khung đồng thuận).";
  return "Thị trường đang giằng co/đi ngang.";
}
// ------------------------------
...
export function buildMarketOutlookV1(snapshot) {
  ...
  const confBase = clamp01(baseQ + regBoost + liqBoost + ofBoost);
    ...
    function commonRisksFor(atrPct, horizonKey, horizonBias, htfBias) {
    const out = [];
    const vol = volatilityRiskVN(atrPct);
    if (vol?.text) out.push(vol.text);

    // Range / chop risk
    if ((regime?.label === "range") || horizonBias === "range") {
      out.push("Thị trường đang range, dễ quét 2 đầu (đặc biệt khi vào lệnh sớm).");
    }

    // HTF conflict risk (intraday vs daily)
    if (htfBias && horizonBias && htfBias !== "range" && horizonBias !== "range" && htfBias !== horizonBias) {
      out.push("Khung ngày đang ngược chiều với khung thấp, dễ giật ngược (tránh vào market).");
    }

    if (quality !== "ok") {
      out.push(quality === "partial" ? "Thiếu một phần dữ liệu, nên giảm khối lượng." : "Thiếu dữ liệu, hạn chế vào lệnh.");
    }
    ...
    // Liquidation risk
    if (liqSum.level === "high") out.push("Có sóng quét mạnh, ưu tiên kỷ luật stop và tránh FOMO.");
    if (liqSum.level === "mid") out.push("Có quét nhẹ, giá có thể rung lắc trước khi đi hướng rõ.");
  ...
  const trendsForHeadline = {
    t15: pickTfFeat(tfFeatures, "15")?.labels?.trend || null,
    t60: pickTfFeat(tfFeatures, "60")?.labels?.trend || null,
    t240: pickTfFeat(tfFeatures, "240")?.labels?.trend || null,
    tD: pickTfFeat(tfFeatures, "D")?.labels?.trend || null,
  };
  const marketPositionText = buildMarketPositionVN(regime?.label || null, trendsForHeadline);

  const headline = {
    market_position: marketPositionText ? `${marketPositionText}` : "Thị trường đang: Chưa rõ.",
  ...
  const h0 = buildRetailOutlookBlock({
    ...
    risks: commonRisksFor(atrPct60 ?? atrPct15, "h0_4h", biasH0, biasH2),
  ...
  const h1 = buildRetailOutlookBlock({
    ...
    strength,
    drivers: driversMid(),
    risks: commonRisksFor(atrPct240, "h1_3d", biasH1, biasH2),
  ...
  const h2 = buildRetailOutlookBlock({
    ...
    strength,
    drivers: driversLong(),
    risks: commonRisksFor(atrPctD, "h2_2w", biasH2, biasH2),
    playbook: playbookFor("h2_2w"),
  ...
  for (let i = 0; i < merged.length; i++) {
    ...
    // Dedupe setups that appear in both top_candidates and candidates_all
    const epSig = safeNum(s.entry_preferred ?? s.entry);
    const stSig = safeNum(s.stop ?? s.invalidation);
    const sig = [s.symbol, s.type || '', s.bias || '', String(s.timeframe || ''),
      (Number.isFinite(epSig) ? epSig.toFixed(6) : 'na'),
      (Number.isFinite(stSig) ? stSig.toFixed(6) : 'na')].join('|');
  ...
  const by_horizon = {
    h0_4h: { focus: [], watchlist: [], avoid: [], note: "Ưu tiên LIMIT trong vùng entry, không đuổi giá." },
    h1_3d: { focus: [], watchlist: [], avoid: [], note: "Ưu tiên kèo theo xu hướng 4H/D, tránh vào lúc giật mạnh." },
    h2_2w: { focus: [], watchlist: [], avoid: [], note: "Ưu tiên theo xu hướng ngày, hạn chế bắt đáy/đỉnh." },
  };
  ...
  for (const bk of ["h0_4h","h1_3d","h2_2w"]) {
    ...
    for (const it of ranked) {
      ...
      const hardAvoidReason =
        rs.includes("price_beyond_invalidation") ||
        rs.includes("not_tradable") ||
        rs.includes("ref_price_missing");

      if (phase === "invalidated" || hardAvoidReason) {
        avoid.push(it.id);
        continue;
      }
      ...
      if (phase === "waiting") {
        watchlist.push(it.id);
        continue;
      }

      if (phase === "missed") {
        // Missed because price is far / avoid chasing => still a valid idea, just WAIT for pullback.
        if (rs.includes("price_too_far_from_entry_zone") || rs.includes("avoid_chasing")) watchlist.push(it.id);
        else avoid.push(it.id);
        continue;
      }
    ...
    by_horizon[bk].watchlist = watchlist.slice(0, 5);
