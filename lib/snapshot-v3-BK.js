...
function deriveTrendState(lastClose, ema, rsi14) {
    ...
    let rsiBias = "rsi_neutral";
    if (rsi14 != null) {
        if (rsi14 >= 60) rsiBias = "rsi_bull";
        else if (rsi14 <= 40) rsiBias = "rsi_bear";
    }

    // Một số state chính, ưu tiên dễ đọc
    ...
    if (above200 && emaStack === "bull_stack" && rsiBias === "rsi_bull") {
        return "strong_bull_trend";
    }
    if (!above200 && emaStack === "bear_stack" && rsiBias === "rsi_bear") {
...
function buildKlinesCompact(klinesRaw, config) {
    ...
    for (const tf of Object.keys(cfg)) {
        ...
        const n = cfg[tf];
        const slice = raw.slice(-n); // lấy N nến cuối
        const parsed = parseKlinesList(slice);

        // Giữ schema gọn: ts / o / h / l / c / v
...
export async function buildSnapshotV3(symbols) {
    ...
    for (const sym of symbols) {
        ...
        // Klines compact cho pattern nến (ít nến, dùng để soi cấu trúc tay)
