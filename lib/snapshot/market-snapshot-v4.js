...
export async function buildMarketSnapshotV4(symbol, { tz = "America/Los_Angeles" } = {}) {
    ...
    async function fetchExchange(exchange) {
        ...
        if (exchange === "bybit") {
            liq = await safe(`${exchange}.liqWS`, () => collectBybitLiquidations(symbol, { windowMs: 4000 }));
        }
        if (exchange === "binance") {
            liq = await safe(`${exchange}.liqWS`, () => collectBinanceLiquidations(symbol, { windowMs: 4000 }));
        }
        ...
        return {
            ...
            derivatives: {
                ...
                liquidations_window: liq.data, // {window_ms, by_side{LONG,SHORT}, events}
            },
            orderflow: {
                orderbook: book.data,
                trades: trades.data,
