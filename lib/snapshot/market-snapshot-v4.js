...
import { buildSetupsV1 } from "../indicators/setup-engine";
...
function buildSweepPrimitiveFromGrab({ liquidity_grab, klines, tf }) {
...
export async function buildMarketSnapshotV4(symbol, { tz = "America/Los_Angeles" } = {}) {
  ...
  for (const tf of INTERVALS) {
    ...
    const sweep_last_p = buildSweepPrimitiveFromGrab({ liquidity_grab: grabTf, klines: k, tf });

    anchor_layer.structure.by_tf[tf] = {
      swing_last: swing_last_p,
      bos_last: bos_last_p,
      // keep raw summaries for backward / debugging
      swings_last,
      bos_last: bosTf.last,
