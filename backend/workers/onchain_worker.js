    ...
    while (start <= toBlock) {
        ...
        try {
            ...
            if ((isLimit || isRate) && chunk > MIN_CHUNK) {
                ...
                await sleep(700);
                continue; // thử lại cùng start với chunk nhỏ hơn
            }

            // nếu đã tối thiểu rồi vẫn bị limit → bỏ qua đoạn này để không kẹt
...
async function fetchFromRPC(coin) {
    ...
    // bắt đầu vừa phải; gặp limit sẽ tự shrink
    const logs = await getTransferLogsInRange(rpcUrl, address, fromBlock, toBlock, 4000);
