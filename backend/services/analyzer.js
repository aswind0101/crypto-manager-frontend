...
export async function analyzeCoin(symbol) {
    ...
    const atr = await getATR14(coin_id); // coin_id đã có ở trên trong scope của bạn
    // fallback an toàn nếu thiếu dữ liệu => dùng % như cũ
    const ATR_OK = Number.isFinite(atr) && atr > 0;

    let buy_zone_min = null;
    let buy_zone_max = null;
    let stop_loss = null;
    let take_profit_1 = null;
    let take_profit_2 = null;

    // SELL/STRONG_SELL: không hiển thị Buy Zone; dùng re-entry zone (ở dưới phần data)
    let reentry_zone = null;

    if (action === "SELL" || action === "STRONG_SELL") {
        // chờ chiết khấu rồi mới mua lại
        if (ATR_OK) {
    ...
    // === lưu vào DB (giữ nguyên lệnh INSERT/UPDATE đang có), chỉ cần cập nhật biến truyền vào ===
    // ... các trường overall_score, action, confidence như cũ ...
    // khi build object `data` để lưu vào cột jsonb, thêm reentry_zone + atr:
    const dataForDb = { px, whale, news, rsi, ema12, ema26, macd, atr, reentry_zone };
    // sau đó dùng dataForDb ở chỗ bạn đang truyền `data` cho INSERT/UPDATE.
