...
function Dashboard() {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => fetchPortfolioWithRetry(user.uid), 120000); // 120s
    ...
    const fetchPortfolioWithRetry = async (userId, retryCount = 0) => {
        try {
            ...
            // Only update if current value is not zero
            if (totalValue > 0) {
                ...
                console.warn("Skipped update because current value is 0");
            }
        } catch (error) {
    ...
    return (
        <div className="p-0 max-w-5xl mx-auto">
            ...
            <div className="mt-4 grid grid-cols-1 gap-4 p-6 rounded-xl shadow-lg bg-black">
                ...
                {loading ? (
                    <p className="text-gray-400 animate-pulse text-center mt-6">Loading portfolio...</p>
                ) : filteredPortfolio.length === 0 ? (
                    <p className="text-gray-400 text-center mt-6">No coins match your search/filter.</p>
                ) : (
                            filteredPortfolio.map((coin, index) => {
                                ...
                                const priceChangeText = avgPrice > 0 && coin.current_price > 0 ? (
                                ...
                                ) : <span className="ml-2 text-xs text-gray-500">Price unavailable</span>;
