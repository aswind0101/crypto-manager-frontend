...
function SetupCard({ setup, onOpen, dense = false, isWide, isMid }) {
    ...
    const isCompact = !isMid; // phone
    const metricsCols = (isWide || isMid) ? 4 : 2;

    // Tile: gọn hơn trên mobile + center content chắc chắn
    const tileStyle = {
        ...
        gap: isCompact ? 2 : 4,
        minWidth: 0,
        width: "100%", // quan trọng: tile lấp đầy cell -> không bị “dạt trái”
    };

    const tileLabel = { fontSize: isCompact ? 10.5 : 11, fontWeight: 750, color: "rgb(71,85,105)" };
    const tileMain = { marginTop: 3, fontSize: isCompact ? 12.5 : 13, fontWeight: 800, color: "rgb(15,23,42)", overflowWrap: "anywhere", minWidth: 0 };
    const tileSub = { marginTop: 3, fontSize: isCompact ? 11.5 : 12, color: "rgb(71,85,105)", fontWeight: 650, overflowWrap: "anywhere", minWidth: 0 };
